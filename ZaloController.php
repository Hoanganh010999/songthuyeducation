...
class ZaloController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        if (empty($message) && empty($mediaUrl) && empty($stickerData)) {
            Log::error('[ZaloController] message, media_url, and sticker_data are all empty', [
                'account_id' => $accountId,
                'recipient_id' => $recipientId,
            ...
            return response()->json([
                'success' => false,
                'message' => 'Either message, media_url, or sticker_data is required',
            ], 400);
        }
        ...
        try {
            ...
            // Save to database
            Log::info('[ZaloController] Saving message to database', [
                'account_id' => $account->id,
                'recipient_id' => $recipientId,
            ...
            // MULTI-SESSION: Let WebSocket listener handle saving (keep architecture consistent)
            // WebSocket will receive the self-sent message and save it with CDN URL
            ...
            // Create a temporary message object for response (not saved to DB yet)
            $savedMessage = (object)[
            ...
            Log::info('[ZaloController] Message sent and saved', [
                'account_id' => $accountId,
                'recipient_id' => $recipientId,
                'recipient_type' => $recipientType,
                'content_type' => $finalContentType,
                'message_id' => $savedMessage?->id,
                'saved_to_db' => !is_null($savedMessage),
            ]);
    ...
    /**
     * Upload image for Zalo message
     * This endpoint uploads an image file and returns a public URL
     */
    public function uploadImage(Request $request)
    {
        ...
        Log::info('[ZaloController] uploadImage: Request inputs', [
            'account_id' => $accountId,
        ...
        // Validate required fields
        // Check both input() and all() to catch FormData fields
        if (!$accountId) {
            // Try to get from all inputs (FormData might be in different location)
        ...
        if (!$request->hasFile('image')) {
            Log::error('[ZaloController] uploadImage: image file is missing', [
                'has_file' => $request->hasFile('image'),
                'all_files' => $request->allFiles(),
                'all_file_keys' => array_keys($request->allFiles()),
                'all_inputs' => $request->all(),
        ...
        // Log file details
        Log::info('[ZaloController] uploadImage: File received', [
        ...
        if (!in_array($fileMimeType, $allowedMimes)) {
            Log::error('[ZaloController] uploadImage: Invalid file type', [
            ...
            return response()->json([
                'success' => false,
                'message' => 'Invalid image type. Allowed: JPEG, PNG, GIF, WebP, AVIF. Received: ' . $fileMimeType,
            ], 400);
        }
        ...
        try {
            ...
            // Path: storage/app/public/zalo/images/{account_id}/{timestamp}_{filename}
            $timestamp = now()->format('YmdHis');
            $filename = $timestamp . '_' . $file->getClientOriginalName();
            $path = $file->storeAs("zalo/images/{$account->id}", $filename, 'public');

            // Generate public URL for frontend display
            $url = asset('storage/' . $path);
            
            // Generate absolute file path for zalo-service to access directly (avoid localhost download)
            $absolutePath = storage_path('app/public/' . $path);

            Log::info('[ZaloController] Image uploaded successfully', [
                'account_id' => $account->id,
                'recipient_id' => $recipientId,
                'recipient_type' => $recipientType,
                'file_path' => $path,
            ...
            return response()->json([
                ...
                'data' => [
                    'url' => $url, // Public URL for frontend display
                    'absolute_path' => $absolutePath, // For zalo-service direct access
                    'path' => $path,
                    'filename' => $filename,
                    'size' => $file->getSize(),
                    'mime_type' => $file->getMimeType(),
    ...
    public function uploadVideo(Request $request)
    {
        ...
        try {
            ...
            // For now, we'll use default values - in production, you'd use ffmpeg or similar
            $duration = 0;
            $width = 0;
            $height = 0;
        ...
        try {
            ...
            if ($isSelf) {
                ...
                $savedMessage = $messageService->saveReceivedMessage(
                    ...
                    $senderId, // Sender Zalo user ID (for group messages)
                    $senderName, // Sender display name (for group messages)
                    $stickerData, // Sticker metadata (for sticker messages)
                    $fileData, // File metadata (for file messages)
                    $styles // Pass styles for rich text
                );
            }
    ...
    public function getReactions(Request $request, $messageId)
    {
        ...
        try {
            ...
            // Get reactions
            $reactions = ZaloMessageReaction::where('zalo_message_id', $message->id)
                ->orderBy('reacted_at', 'desc')
                ->get()
                ->groupBy('reaction_icon')
                ->map(function ($group) {
                    return [
                        'reaction' => $group->first()->reaction_icon,
                        'count' => $group->count(),
    ...
    public function recallMessage(Request $request, $id)
    {
        try {
            ...
            $response = Http::timeout(30)->withHeaders([
                'X-API-Key' => $apiKey,
                'X-Account-Id' => $message->account_id,
