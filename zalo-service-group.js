...
router.get('/list', verifyApiKey, async (req, res) => {
  try {
      ...
      if (groups.gridVerMap && typeof groups.gridVerMap === 'object') {
        ...
        for (let i = 0; i < maxGroups; i++) {
          ...
          try {
            ...
            console.log(`   ⚠️  Failed to get info for group ${groupId}:`, error.message);
        ...
        console.log(`   ✅ Fetched ${groupsList.length} group details`);
    ...
    const normalizedGroups = await Promise.all(groupsList.map(async (group) => {
      ...
      // DEBUG: Log what we have for this group      if (groupId === '937352231569092258') {        console.log();        console.log();        console.log();        console.log();        console.log();        console.log();        console.log();      }
      let membersCount = 0;
  ...
  try {
    ...
    if (groupInfoRaw && groupInfoRaw.gridInfoMap) {
      ...
      if (gridInfoValues.length > 0) {
        ...
        console.log('   ✅ Extracted group info from gridInfoMap');
      }
    } else {
