...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::prefix('lesson-plans')->group(function () {
        ...
        Route::put('/{id}/sessions/reorder', [LessonPlanController::class, 'reorderSessions']); // Reorder sessions
...
Route::middleware(['auth:sanctum'])->prefix('course')->group(function () {
    ...
    Route::prefix('learning-journals')->group(function () {
        ...
        Route::delete('/{journalId}', [\App\Http\Controllers\Api\LearningJournalController::class, 'deleteJournal']); // 🔥 NEW: Delete journal
    ...
    // Vocabulary Book (Personal)
    Route::prefix('vocabulary')->group(function () {
        Route::get('/', [VocabularyController::class, 'index']); // Get user's vocabulary
        Route::post('/', [VocabularyController::class, 'store']); // Add new entry
        Route::put('/{id}', [VocabularyController::class, 'update']); // Update entry
        Route::delete('/{id}', [VocabularyController::class, 'destroy']); // Delete entry
        
        // Flashcard review
        Route::get('/review/random', [VocabularyController::class, 'getRandomForReview']); // Get random words for review
        Route::post('/{id}/review', [VocabularyController::class, 'recordReview']); // Record review result
        
        // Audio pronunciation
        Route::post('/{id}/pronunciation-audio', [VocabularyController::class, 'uploadPronunciationAudio']); // Upload pronunciation recording
        
        // Statistics
        Route::get('/statistics', [VocabularyController::class, 'getStatistics']); // Get vocabulary stats
    });
...
// Route aliases for backward compatibility with frontend (legacy /api/message/* paths)
Route::prefix('message')->middleware(['auth:sanctum', 'branch.access'])->group(function () {
    Route::post('/send-video', [\App\Http\Controllers\Api\ZaloController::class, 'uploadVideo'])->middleware('permission:zalo.send');
    Route::post('/send-voice', [\App\Http\Controllers\Api\ZaloController::class, 'uploadAudio'])->middleware('permission:zalo.send');
