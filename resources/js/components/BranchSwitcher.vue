...
const loadUserBranches = async () => {
  ...
  try {
    ...
    // Set current branch from user data first, then localStorage
    if (authStore.currentUser?.current_branch_id) {
      // Backend provided current_branch_id (students, parents, employees)
      currentBranchId.value = authStore.currentUser.current_branch_id;
      localStorage.setItem('current_branch_id', authStore.currentUser.current_branch_id);
    } else {
      // Check if there's a valid saved branch in localStorage
      ...
      if (savedBranchId && userBranches.value.some(b => b.id == savedBranchId)) {
        ...
        // Only auto-select first branch for employees with multiple branches
        // Students/parents should NOT reach here as they have current_branch_id from backend
        currentBranchId.value = userBranches.value[0].id;
        localStorage.setItem('current_branch_id', userBranches.value[0].id);
      } else {
        // No branches available - clear localStorage
