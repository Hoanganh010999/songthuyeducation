...
export function useI18n() {
    ...
    const loadTranslations = async (languageCode) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                // Check if we need to refresh (new keys available)
                const cachedTranslations = localStorage.getItem('app_translations');
                if (cachedTranslations) {
                    try {
                        const cached = JSON.parse(cachedTranslations);
                        const cachedKeys = Object.keys(cached.settings || {}).length;
                        const newKeys = Object.keys(newTranslations.settings || {}).length;
                        
                        if (newKeys > cachedKeys) {
                            console.log(`🔄 Detected new translations: ${newKeys - cachedKeys} new keys in settings group`);
                        }
                    } catch (e) {
                        console.warn('Could not parse cached translations for comparison');
                    }
                }
                ...
                console.log('📊 Settings keys count:', Object.keys(translations.value.settings || {}).length);
                ...
                localStorage.setItem('app_translations_version', Date.now().toString());
    ...
    const initI18n = async () => {
        ...
        // Use cached translations TEMPORARILY while loading fresh ones
        if (savedLanguage && savedTranslations) {
            try {
                ...
                console.log('📦 Using cached translations temporarily for:', savedLanguage);
            } catch (error) {
                console.error('Failed to parse cached translations:', error);
            }
        }

        // ALWAYS load fresh translations from API (this will update cache)
        ...
        console.log('📊 Settings keys:', Object.keys(translations.value.settings || {}).length);
    };
    ...
    const t = (key, params = null) => {
        // Split only at the FIRST dot to get group and the rest as translationKey
        ...
        const group = key.substring(0, firstDotIndex);
        const translationKey = key.substring(firstDotIndex + 1);
