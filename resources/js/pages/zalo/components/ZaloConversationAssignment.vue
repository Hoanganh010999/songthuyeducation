<template>
  <div class="space-y-4">
    ...
    <!-- Current Branch Assignment (for conversations and friends, not groups) -->
    <div v-if="canAssignBranch && (itemType === 'conversations' || itemType === 'friends')" class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        >
          {{ conversation.assigned_branch_id ? t('zalo.change') : t('zalo.assign') }}
        </button>
      </div>
      <div v-if="conversation.assigned_branch" class="flex items-center gap-2 p-2 bg-purple-50 rounded">
        <svg class="w-4 h-4 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
        </svg>
        <span class="text-sm font-medium text-gray-900">{{ conversation.assigned_branch?.name }}</span>
      </div>
      <div v-else class="text-sm text-gray-400 italic">{{ t('zalo.unassigned') }} ({{ t('zalo.global') || 'Global' }})</div>
    </div>
    ...
    <div class="space-y-2">
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          <button
            v-if="conversation.assigned_users && conversation.assigned_users.length > 0"
            @click="removeAllUserAssignments"
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
          ...
          <div class="p-4">
            ...
            <div v-else class="space-y-2 max-h-96 overflow-y-auto">
              <!-- Unassign option -->
              <button
                @click="assignDepartment(null)"
                class="w-full text-left p-3 rounded-lg hover:bg-gray-50 border border-gray-200"
                :class="!conversation.department ? 'bg-gray-100 border-gray-400' : ''"
              >
                <div class="font-medium text-gray-900">{{ t('zalo.unassigned') }}</div>
                <div class="text-xs text-gray-500 mt-1">{{ t('zalo.not_assigned_department') }}</div>
              </button>
      ...
      <div v-if="showBranchModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
          ...
          <div class="p-4">
            ...
            <div v-else class="space-y-2 max-h-96 overflow-y-auto">
              <!-- Unassign option (Global) -->
              <button
                @click="assignBranch(null)"
                class="w-full text-left p-3 rounded-lg hover:bg-gray-50 border border-gray-200"
                :class="!conversation.assigned_branch_id ? 'bg-gray-100 border-gray-400' : ''"
              >
              ...
              <button
                ...
                class="w-full text-left p-3 rounded-lg hover:bg-gray-50 border border-gray-200"
                :class="conversation.assigned_branch_id === branch.id ? 'bg-purple-50 border-purple-300' : ''"
              >
...
// Check if user can assign branches (superadmin or has zalo.view_all_branches_conversations permission)
const canAssignBranch = computed(() => {
  return authStore.isSuperAdmin || authStore.hasPermission('zalo.view_all_branches_conversations');
});
...
const removeAllUserAssignments = async () => {
  const result = await useSwal().fire({
    icon: 'warning',
    title: t('common.confirm'),
    text: t('zalo.remove_all_assignments_confirm') || 'Bạn có chắc muốn bỏ gán tất cả nhân viên?',
    showCancelButton: true,
    confirmButtonText: t('common.delete'),
    cancelButtonText: t('common.cancel'),
  });
  ...
  try {
    // Remove each user assignment
    const userIds = props.conversation.assigned_users?.map(u => u.id) || [];
    const conversationDbId = props.conversation.conversation_id || props.conversation.id;

    let successCount = 0;
    for (const userId of userIds) {
      try {
        await axios.delete(`/api/zalo/conversations/${conversationDbId}/assign-user/${userId}`);
        successCount++;
      } catch (error) {
    ...
    if (successCount > 0) {
      useSwal().fire({
        icon: 'success',
        title: t('common.success'),
        text: `Đã bỏ gán ${successCount}/${userIds.length} nhân viên`,
        timer: 2000,
      });

      // Clear local conversation assigned users
      props.conversation.assigned_users = [];
      emit('updated');
    }
  } catch (error) {
    console.error('Failed to remove all user assignments:', error);
    useSwal().fire({
      icon: 'error',
      title: t('common.error'),
      text: error.response?.data?.message || t('zalo.cannot_remove_assignment'),
...
// Load available branches for this conversation (based on Zalo account sharing)
const loadBranches = async () => {
  loadingBranches.value = true;
  try {
    // Get branches that share the same Zalo account as this conversation
    const accountId = localStorage.getItem('active_zalo_account_id');
    const conversationDbId = props.conversation.conversation_id || props.conversation.id;
    const response = await axios.get(`/api/zalo/conversations/${conversationDbId}/available-branches`, {
      params: {
        account_id: accountId
      }
    });
    branches.value = response.data.data || [];
    console.log('✅ [ZaloConversationAssignment] Loaded branches:', branches.value.length);
  } catch (error) {
    console.error('❌ [ZaloConversationAssignment] Failed to load branches:', error);
...
const assignBranch = async (branchId) => {
  try {
    const accountId = localStorage.getItem('active_zalo_account_id');
    const conversationDbId = props.conversation.conversation_id || props.conversation.id;
    const response = await axios.put(`/api/zalo/conversations/${conversationDbId}/assign-branch`, {
      branch_id: branchId,
      account_id: accountId,
    });

    if (response.data.success) {
      // Update local conversation object from backend response
      const updatedConversation = response.data.data;
      if (updatedConversation) {
        props.conversation.assigned_branch_id = updatedConversation.assigned_branch_id;
        props.conversation.assigned_branch = updatedConversation.assigned_branch;

        console.log('✅ [ZaloConversationAssignment] Conversation updated from backend:', {
          assigned_branch_id: updatedConversation.assigned_branch_id,
          assigned_branch: updatedConversation.assigned_branch,
        });
      }
      ...
      useSwal().fire({
        icon: 'success',
        title: t('common.success'),
        text: t('zalo.branch_assigned_success') || 'Branch assigned successfully',
        timer: 1500,
      });

      showBranchModal.value = false;

      // Emit updated event with the full updated conversation data
      emit('updated', updatedConversation);
    }
  } catch (error) {
    console.error('❌ [ZaloConversationAssignment] Failed to assign branch:', error);
    useSwal().fire({
      icon: 'error',
      title: t('common.error'),
