<template>
  <div class="flex flex-col h-full overflow-hidden">
      ...
      <template v-else>
        ...
        >
          ...
          >
            ...
            <!-- Message bubble (ONLY for non-file/folder/video/audio messages or when has quote) -->
            <div
              v-if="(message.content_type !== 'file' && message.content_type !== 'folder' && message.content_type !== 'video' && message.content_type !== 'audio') || message.reply_to || message.quote_data"
            ...
            >
              ...
              <!-- Regular message content - hide for special messages, files, folders, videos, audios, stickers, images -->
              <div
                v-else-if="message.content_type !== 'file' && message.content_type !== 'folder' && message.content_type !== 'video' && message.content_type !== 'audio' && message.content_type !== 'sticker' && message.content_type !== 'image'"
            ...
            >
              <a
                ...
                :class="message.type === 'sent' ? 'bg-slate-50/30 border-slate-100' : 'bg-gray-50 border-gray-200'"
              >
                <!-- Folder icon -->
            ...
            <a
              ...
              target="_blank"
              download
              class="flex items-center gap-3 mt-2 px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors group"
            >
              <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-red-500 text-white group-hover:bg-red-600 transition-colors">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500">{{ $t('zalo.video_sent') }}</p>
              </div>
            ...
            <a
              v-if="message.content_type === 'audio' && (message.media_url || message.metadata?.file?.href)"
              :href="message.media_url || message.metadata?.file?.href"
              target="_blank"
              download
              class="flex items-center gap-3 mt-2 px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors group"
            >
              <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-purple-500 text-white group-hover:bg-purple-600 transition-colors">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-500">{{ $t('zalo.click_to_listen') }}</p>
              </div>
              <svg class="w-5 h-5 text-gray-400 group-hover:text-gray-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
              </svg>
            ...
            >
              <!-- Timestamp + Sender info (for sent messages) -->
              <div class="flex items-center gap-2 text-xs text-gray-500">
                <span>{{ formatTime(message.sent_at || message.created_at) }}</span>
                <!-- Sender info: User name - Account username (sent messages only) -->
                <span 
                  v-if="message.type === 'sent' && (message.sent_by_user_name || message.sent_by_account_username)"
                  class="text-gray-400"
                >
                  • {{ message.sent_by_user_name }}<template v-if="message.sent_by_user_name && message.sent_by_account_username"> - {{ extractUsername(message.sent_by_account_username) }}</template>
                </span>
              </div>
    ...
    <!-- Input area -->
    <div class="px-6 py-4 border-t border-gray-200 bg-white flex-shrink-0 relative">
      <!-- Tools row (top) -->
      <div class="flex items-center gap-2 pb-2 border-b border-gray-100">
        ...
        <!-- Image upload button (ZaloImageUploader component) -->
        <ZaloImageUploader
          :account-id="currentAccountId"
          :recipient-id="props.itemType === 'groups' ? (props.item.zalo_group_id || props.item.id) : (props.item.zalo_user_id || props.item.id)"
          :recipient-type="props.itemType === 'groups' ? 'group' : 'user'"
          @images-sent="handleImagesSent"
        />
        <!-- Video upload button -->
      ...
      <!-- Rich text editor (Quill with built-in toolbar) -->
      <div v-if="showEditor" class="mb-2">
        <div ref="editorContent"></div>
      </div>

      <!-- Mention Picker (shared for both textarea and editor) -->
      <div
        v-if="showMentionPicker && props.itemType === 'groups'"
        class="absolute bg-white border border-gray-300 rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto"
        :style="{ bottom: '80px', left: '30px', minWidth: '250px' }"
      >
        <div
          v-for="(member, index) in filteredMembers"
          :key="member.zalo_user_id || index"
        ...
        >
          ...
          <div v-else class="w-8 h-8 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm font-semibold">
            {{ getInitials(member.name) }}
          </div>
          <span class="text-sm">{{ member.name }}</span>
        </div>
        <div v-if="filteredMembers.length === 0" class="px-4 py-2 text-sm text-gray-500">
...
import ReactionsModal from '../../../components/zalo/ReactionsModal.vue';
import Quill from 'quill';
import 'quill/dist/quill.snow.css';
import ZaloImageUploader from '../../../components/zalo/ZaloImageUploader.vue';
...
const quillReady = ref(false); // Flag to indicate Quill is fully ready (after warmup)
...
const getMediaTitle = (message) => {
  ...
  if (message.content_type === 'audio') return 'Audio';
...
const formatMessageContent = (content, contentType, asHtml = false) => {
  ...
  // Check if content is JSON (file/image/video/audio metadata)
...
const formatMessageContentWithStyles = (content, contentType, styles) => {
  ...
  // Styles can be in TWO formats:
  // 1. NEW format: [{ start: number, len: number, st: string }] - from Zalo API
  // 2. OLD format: [{ start: number, end: number, bold?: boolean, italic?: boolean, ... }]
  const plainText = formatMessageContent(content, contentType, false);
  if (!plainText) return html;

  // Debug log removed

  // Normalize styles to old format for consistent processing
  const normalizedStyles = styles.map(style => {
    // If style has 'st' field (NEW format), convert it
    if (style.st) {
      const normalized = {
        start: style.start || 0,
        // Calculate end from len (NEW format) or use end (OLD format)
        end: style.len !== undefined ? (style.start || 0) + style.len : (style.end || plainText.length)
      };

      const st = style.st;
      if (st === 'lst_1') normalized.unorderedList = true;
      else if (st === 'lst_2') normalized.orderedList = true;
      else if (st === 'b') normalized.bold = true;
      else if (st === 'i') normalized.italic = true;
      else if (st === 'u') normalized.underline = true;
      else if (st === 's') normalized.strikethrough = true;
      else if (st.startsWith('c_')) normalized.color = st; // Color codes like c_db342e

      return normalized;
    }

    // OLD format - just ensure end is calculated
    return {
      ...style,
      end: style.end !== undefined ? style.end : (style.len !== undefined ? (style.start || 0) + style.len : plainText.length)
    };
  });

  // Debug log removed

  // Check if ANY styles are list items (not just all)
  const hasAnyListStyles = normalizedStyles.some(s => s.unorderedList || s.orderedList);

  if (hasAnyListStyles) {
    // Has list styles - group by position and render in chronological order
    // Group styles by start position
    const stylesByPosition = new Map();

    normalizedStyles.forEach((style) => {
      const key = `${style.start}-${style.end}`;
      if (!stylesByPosition.has(key)) {
        stylesByPosition.set(key, {
          start: style.start,
          end: style.end,
          styles: []
        });
      }
      stylesByPosition.get(key).styles.push(style);
    });

    // Sort by position - KEEP chronological order
    const sortedPositions = Array.from(stylesByPosition.values())
    ...
    const renderStyledText = (text, styles) => {
      const colorStyle = styles.find(s => s.color);
      const boldStyle = styles.find(s => s.bold);
      const italicStyle = styles.find(s => s.italic);
      const underlineStyle = styles.find(s => s.underline);
      const strikethroughStyle = styles.find(s => s.strikethrough);

      const classes = [];
      const styleAttrs = [];

      if (boldStyle) classes.push('font-bold');
      if (italicStyle) classes.push('italic');
      if (underlineStyle) classes.push('underline');
      if (strikethroughStyle) classes.push('line-through');
      ...
      if (colorStyle && colorStyle.color) {
        ...
        const color = colorMap[colorStyle.color];
        if (color) {
          styleAttrs.push(`color: ${color}`);
        }
      }

      let itemHtml = escapeHtml(text);
      if (classes.length > 0 || styleAttrs.length > 0) {
        itemHtml = `<span${classes.length > 0 ? ` class="${classes.join(' ')}"` : ''}${styleAttrs.length > 0 ? ` style="${styleAttrs.join('; ')}"` : ''}>${itemHtml}</span>`;
      }

      return itemHtml;
    };

    // Iterate through positions in chronological order
    let lastProcessedEnd = 0;
    
    sortedPositions.forEach((position, index) => {
      const start = position.start || 0;
      const end = position.end || plainText.length;
      
      // ✅ RENDER TEXT PLAIN GIỮA CÁC STYLED SECTIONS
      if (start > lastProcessedEnd) {
        const rawGapText = plainText.substring(lastProcessedEnd, start);
        // Split by newlines and render each line
        const gapLines = rawGapText.split('\n').map(line => line.trim()).filter(line => line);
        
        if (gapLines.length > 0) {
          // Close any open list before rendering plain text
          if (inList) {
            listHtml += `</${listType}>`;
            finalHtml += listHtml;
            inList = false;
          ...
          // Render each line with <br> between them
          finalHtml += gapLines.map(line => escapeHtml(line)).join('<br>') + '<br>';
        }
      }
      
      const hasListStyle = position.styles.some(s => s.unorderedList || s.orderedList);
      let itemText = plainText.substring(start, end).trim();
      ...
      if (hasListStyle) {
        ...
        if (!inList) {
          // Start a new list
          const firstListStyle = position.styles.find(s => s.unorderedList || s.orderedList);
          listType = firstListStyle?.unorderedList ? 'ul' : 'ol';
          listHtml = `<${listType} class="message-list">`;
          inList = true;
        }

        // Add list item
        const itemHtml = renderStyledText(itemText, position.styles);
        listHtml += `<li>${itemHtml}</li>`;

        // Debug log removed

        // Check if next item is also a list item
        const nextPosition = sortedPositions[index + 1];
        const nextHasListStyle = nextPosition?.styles.some(s => s.unorderedList || s.orderedList);
        ...
        // ✅ UPDATE lastProcessedEnd for list items too
        lastProcessedEnd = end;
      } else {
        // This is non-list content
        if (inList) {
        ...
        const itemHtml = renderStyledText(itemText, position.styles);
        finalHtml += itemHtml + '<br>';  // ✅ Thêm <br> để xuống dòng

        // Debug log removed
      }
      
      // ✅ UPDATE lastProcessedEnd sau mỗi position
      lastProcessedEnd = end;
    });
    ...
    // Check for any text AFTER all styled content
    const maxStyledPosition = Math.max(...normalizedStyles.map(s => s.end || 0));
    if (maxStyledPosition < plainText.length) {
      const remainingText = plainText.substring(maxStyledPosition).trim();
      if (remainingText) {
        // Debug log removed
        finalHtml += escapeHtml(remainingText);
      }
    }
  ...
  const sortedStyles = [...normalizedStyles].sort((a, b) => (a.start || 0) - (b.start || 0));
...
const loadGroupMembers = async () => {
  if (props.itemType !== 'groups' || !props.item?.id) {
    groupMembers.value = [];
    return;
  }
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.log('👥 [ZaloChatView] Group members loaded:', groupMembers.value.length);
    }
  } catch (error) {
    console.error('❌ [ZaloChatView] Failed to load group members:', error);
    groupMembers.value = [];
  }
};

// Filtered members for mention picker (includes @all)
const filteredMembers = computed(() => {
  ...
  const allOption = {
    zalo_user_id: 'all',
    display_name: 'all', // For @all, just use "all" without @
    name: '@all (Tất cả thành viên)', // Display text in picker
    avatar: null,
    is_all: true
  };

  const members = groupMembers.value.filter(member => {
    const memberId = member.id || member.zalo_user_id || member.uid;
    return member.display_name?.toLowerCase().includes(query) ||
           memberId?.includes(query);
  }).map(member => ({
    id: member.id || member.zalo_user_id || member.uid, // Use 'id' as primary field
    zalo_user_id: member.id || member.zalo_user_id || member.uid, // Keep for backward compatibility
    display_name: member.display_name || 'Thành viên Zalo', // Keep display_name for consistency
    name: member.display_name || 'Thành viên Zalo', // Keep name for backward compatibility
    avatar: member.avatar_url,
    is_all: false
  }));

  // If query is empty or matches "all", show @all first
  if (!query || 'all'.includes(query) || 'tất cả'.includes(query)) {
...
const handleTextareaInput = (event) => {
  console.log('🔍 [ZaloChatView] handleTextareaInput triggered', {
    itemType: props.itemType,
    isGroup: props.itemType === 'groups',
    groupMembersCount: groupMembers.value.length,
    text: event.target.value,
  });

  if (props.itemType !== 'groups') {
    console.log('⚠️ [ZaloChatView] Not a group, skipping mention detection');
    return;
  }
  ...
  console.log('🔍 [ZaloChatView] Checking for @ symbol', { text, cursorPos });
  ...
  for (let i = cursorPos - 1; i >= 0; i--) {
    if (text[i] === '@') {
      ...
      if (i === 0 || /\s/.test(text[i - 1])) {
        atPos = i;
        console.log('✅ [ZaloChatView] Found @ at position:', atPos);
        break;
      }
  ...
  if (atPos !== -1) {
    ...
    console.log('🔍 [ZaloChatView] Query after @:', query, 'hasSpaces:', /\s/.test(query));
    
    // Only show picker if query doesn't contain spaces (still typing)
    if (!/\s/.test(query)) {
      mentionStartPos.value = atPos;
      mentionQuery.value = query;
      showMentionPicker.value = true;
      console.log('👤 [ZaloChatView] Mention picker activated:', { 
        query, 
        atPos, 
        groupMembersCount: groupMembers.value.length,
  ...
  // Hide picker if conditions not met
...
const handleTextareaKeydown = (event) => {
  ...
  // TODO: Add Up/Down arrow navigation for selecting members
};

// Detect @ in Quill Editor for mentions
const handleQuillMentionDetection = () => {
  if (props.itemType !== 'groups' || !quillInstance.value) {
    return;
  }
  ...
  try {
    ...
    console.log('🔍 [ZaloChatView] Quill mention detection', {
      cursorPos,
      text: text.substring(Math.max(0, cursorPos - 20), cursorPos + 5),
      groupMembersCount: groupMembers.value.length,
    });
    ...
    for (let i = cursorPos - 1; i >= 0; i--) {
      if (text[i] === '@') {
        ...
        if (i === 0 || /\s/.test(text[i - 1])) {
          atPos = i;
          console.log('✅ [ZaloChatView] Found @ in Quill at position:', atPos);
          break;
        }
      } else if (/\s/.test(text[i])) {
        // Stop if we hit whitespace before finding @
    ...
    if (atPos !== -1) {
      ...
      console.log('🔍 [ZaloChatView] Query after @ in Quill:', query, 'hasSpaces:', /\s/.test(query));
      ...
      if (!/\s/.test(query)) {
        ...
        mentionQuery.value = query;
        showMentionPicker.value = true;
        console.log('👤 [ZaloChatView] Quill mention picker activated:', { 
          query, 
          atPos, 
          groupMembersCount: groupMembers.value.length,
          filteredCount: filteredMembers.value.length 
    ...
    console.error('❌ [ZaloChatView] Error in handleQuillMentionDetection:', error);
  }
};
...
const insertMention = (member) => {
  ...
  // Use display_name to match parseMentions logic
  const mentionText = member.is_all ? '@all' : `@${member.display_name || member.name}`;
  
  console.log('📝 [ZaloChatView] insertMention called:', {
    member: member,
    mentionText: mentionText,
    is_all: member.is_all,
    display_name: member.display_name,
    name: member.name,
  });
  
  // Handle Quill Editor
  if (showEditor.value && quillInstance.value) {
    try {
      const selection = quillInstance.value.getSelection();
      if (!selection) return;

      // Delete the @query text
      const deleteLength = selection.index - mentionStartPos.value;
      quillInstance.value.deleteText(mentionStartPos.value, deleteLength);
      
      // Insert mention text
      quillInstance.value.insertText(mentionStartPos.value, mentionText + ' ');
      
      // Set cursor after mention
      const newCursorPos = mentionStartPos.value + mentionText.length + 1;
      quillInstance.value.setSelection(newCursorPos);
      
      console.log('✅ [ZaloChatView] Mention inserted in Quill:', mentionText);
    } catch (error) {
      console.error('❌ [ZaloChatView] Error inserting mention in Quill:', error);
    }
  }
  // Handle plain textarea
    ...
    console.log('✅ [ZaloChatView] Mention inserted in textarea:', mentionText);
  }
...
// Rich text editor functions - REWRITTEN FOR QUILL
const toggleEditor = () => {
  console.log('🔵 [toggleEditor] Called. Current showEditor:', showEditor.value);

  if (!showEditor.value) {
    // Bật editor: chuyển text từ textarea vào Quill editor
    console.log('🟢 [toggleEditor] Opening editor (showEditor: false -> true)');
    showEditor.value = true;

    // Wait for Quill to be initialized AND ready (watch will trigger initialization)
    const waitForQuill = setInterval(() => {
      if (quillInstance.value && quillReady.value) {
        clearInterval(waitForQuill);
        console.log('✅ [toggleEditor] Quill is ready, setting up editor...');

        // Set text từ textarea vào Quill (plain text)
        if (messageText.value) {
          quillInstance.value.setText(messageText.value);
        }
        ...
        try {
          ...
          quillInstance.value.setSelection(length - 1, 0);
        } catch (err) {
          console.warn('Could not focus editor:', err);
        }

        console.log('✅ [toggleEditor] Switched to rich text editor');
      }
    }, 50);

    // Timeout after 2 seconds
    setTimeout(() => clearInterval(waitForQuill), 2000);
  } else {
    // Tắt editor: chuyển text từ Quill về textarea (plain text only)
    if (quillInstance.value) {
      const text = quillInstance.value.getText();
      // Remove trailing newline that Quill always adds
      messageText.value = text.endsWith('\n') ? text.slice(0, -1) : text;
    }
    ...
    console.log('✅ [toggleEditor] Switched to simple textarea');
  }
};
...
const handleEditorInput = () => {
  ...
  try {
    ...
    // Note: Toolbar button states are now updated via 'selection-change' event
    // This prevents race conditions when calling getSelection() during format changes

    console.log('📝 [ZaloChatView] Quill content length:', plainText.length);
  } catch (err) {
...
const handleEditorEnter = (event) => {
  ...
  if (listItem && listElement) {
    ...
    // If list item is empty, exit list mode
    if (currentText === '') {
      console.log('📝 [handleEditorEnter] Empty list item detected - exiting bullet mode');
      ...
      if (listElement.children.length === 0) {
        console.log('📝 [handleEditorEnter] List is empty - removing list element');
        listElement.remove();
      }

      // Create a new line after the list for normal text
      // Use <div><br></div> to ensure proper line spacing
      const newLine = document.createElement('div');
      const br = document.createElement('br');
      newLine.appendChild(br);

      // Insert after the list (or where the list was if it was removed)
      if (listElement.parentNode) {
        listElement.parentNode.insertBefore(newLine, listElement.nextSibling);
      } else {
        // List was removed, append to editor
        editorContent.value.appendChild(newLine);
      }
      ...
      console.log('📝 [handleEditorEnter] Exited bullet mode successfully');
      return;
    }
...
const insertBulletList = () => {
  if (!quillInstance.value || !quillReady.value) return;
  ...
  try {
    ...
    console.log('📝 [insertBulletList] Toggled bullet list');
  } catch (err) {
    console.error('❌ [insertBulletList] Error:', err);
  }
};
...
const applyStyle = (style) => {
  ...
  try {
    ...
    // CRITICAL: Blur then focus to completely reset Quill's selection state
    quill.blur();
    quill.focus();

    // Now toggle format - Quill will use fresh selection state
    const currentFormat = quill.getFormat();
    const currentValue = currentFormat[style];
    quill.format(style, !currentValue);

    console.log(`📝 [applyStyle] Toggled ${style}`);
  } catch (err) {
    console.error('❌ [applyStyle] Error:', err);
  }
};
...
const applyColor = (color) => {
  ...
  try {
    ...
    const hexColor = colorHex[color] || colorHex.red;
    ...
    // Toggle color
    const currentFormat = quill.getFormat();
    const currentColor = currentFormat.color;
    quill.format('color', currentColor === hexColor ? false : hexColor);

    console.log(`📝 [applyColor] Applied color: ${color}`);
  } catch (err) {
    console.error('❌ [applyColor] Error:', err);
  }
};
...
// Extract styles from editor content for zalo-api-final format (Using Quill Delta)
const extractStylesFromEditor = () => {
  // NEW: Use Quill Delta instead of HTML parsing
  ...
  // Color hex to Zalo format
  const colorToZaloFormat = (hexColor) => {
    if (!hexColor) return null;
    return `c_${hexColor.replace('#', '')}`;
  };

  // Log removed - only show when sending message via __DEBUG_ZALO_EXTRACT__

  // IMPORTANT: In Quill Delta, list attributes are on the newline, not the text
  // We need to track which lines are lists and apply styles to the text, not newlines
  let lineStartPosition = 0;
  let nextLineWillBeList = false;  // Track if NEXT line will be list item
  let nextListType = null;
  ...
  delta.ops.forEach((op, index) => {
    if (typeof op.insert === 'string') {
      ...
      // IMPORTANT: Quill can combine \n with text in one op (e.g., "\n123456789")
      // We need to split these cases to handle newlines correctly
      if (op.insert.includes('\n') && op.insert.length > 1) {
        // Split by newline and process each part separately
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          // Process text part (if not empty)
          if (part.length > 0) {
            // Apply text formatting styles
            if (Object.keys(attributes).length > 0) {
              if (attributes.bold) {
                zaloApiStyles.push({ start: currentPosition, len: part.length, st: 'b' });
              }
              if (attributes.italic) {
                zaloApiStyles.push({ start: currentPosition, len: part.length, st: 'i' });
              }
              if (attributes.underline) {
                zaloApiStyles.push({ start: currentPosition, len: part.length, st: 'u' });
              }
              if (attributes.strike) {
                zaloApiStyles.push({ start: currentPosition, len: part.length, st: 's' });
              }
              if (attributes.color) {
                const zaloColor = colorToZaloFormat(attributes.color);
                if (zaloColor) {
                  zaloApiStyles.push({ start: currentPosition, len: part.length, st: zaloColor });
                }
              }
            }
          ...
          // Process newline (after each part except the last)
          if (!isLastPart) {
            // This newline ends the current line
            // In Quill, list attribute is typically on the NEXT newline
            // So we don't apply list here, just move to next line
            const lineLength = currentPosition - lineStartPosition;
            if (lineLength > 0 && nextLineWillBeList) {
              zaloApiStyles.push({ start: lineStartPosition, len: lineLength, st: nextListType });
              nextLineWillBeList = false;
            }
        ...
        // Pure newline operation
        const isNewline = true;

        // Handle newlines with list attributes
        if (attributes.list) {
          // This newline indicates the CURRENT line (before this newline) is a list item
          ...
          // DO NOT mark next line as list - only this newline indicates current line is list
          // If next line should also be list, it will have its own newline with list attribute
        } else {
          // Regular newline without list
          const lineLength = currentPosition - lineStartPosition;
          
          // If current line was marked as list, apply it
          if (lineLength > 0 && nextLineWillBeList) {
            zaloApiStyles.push({ start: lineStartPosition, len: lineLength, st: nextListType });
          }
          
          nextLineWillBeList = false;  // Reset flag
        }
        ...
        // Regular text without newlines - apply text formatting styles
  ...
  // Handle last line if it was marked as list item (no trailing newline)
  const lastLineLength = currentPosition - lineStartPosition;
  if (lastLineLength > 0 && nextLineWillBeList) {
    zaloApiStyles.push({ start: lineStartPosition, len: lastLineLength, st: nextListType });
  }

  // Only log during development/debugging - will be shown when sending message
  if (window.__DEBUG_ZALO_EXTRACT__) {
    console.log('📝 [ZaloChatView] Extracted styles (Zalo API format from Quill):', {
      stylesCount: zaloApiStyles.length,
      styles: zaloApiStyles,
      plainText: plainText.substring(0, 100),
      plainTextLength: plainText.length,
      stylesPreview: zaloApiStyles.map(s => ({
        start: s.start,
        len: s.len,
        st: s.st,
        text: plainText.substring(s.start, s.start + s.len).replace(/\n/g, '\\n')
      }))
    });
    
    console.log('🔍 [ZaloChatView] Detailed Delta analysis:', {
      deltaOps: delta.ops.map((op, idx) => ({
        index: idx,
        insert: typeof op.insert === 'string' ? op.insert.replace(/\n/g, '\\n') : op.insert,
        insertLength: typeof op.insert === 'string' ? op.insert.length : 0,
        attributes: op.attributes || {},
        isNewline: op.insert === '\n',
        hasListAttr: !!(op.attributes && op.attributes.list)
      }))
    });
  }
  ...
  // OLD HTML PARSING CODE BELOW (no longer used):
  const html = '';
  const styles = [];
  ...
  // CRITICAL FIX: Append to document so getComputedStyle() works correctly
  // getComputedStyle() returns incorrect values for elements not in DOM
  // Set display: none to prevent visual flicker
  ...
  const traverse = (node, startIndex = 0) => {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      // Check for list (ul/ol) - ONLY apply list style if text is inside <li>
      // This prevents non-list text from getting bullet points
      let listParent = parent;
      let isInsideLI = false;
      
      // First, check if we're inside an <li> element
      ...
      // Only apply list style if we're inside <li>, then check for ul/ol
      if (isInsideLI) {
        listParent = parent;
        while (listParent && listParent !== tempDiv) {
      ...
      // Only add style if there's actual text content
      // This prevents empty bullet points from being sent
      if (hasStyle && text.trim()) {
        styles.push({
          start: currentIndex,
          end: currentIndex + text.length,
          ...style
        });
      } else if (hasStyle && !text.trim() && (style.unorderedList || style.orderedList)) {
        console.log('⚠️ [extractStylesFromEditor] Skipping empty list item at position', currentIndex);
      }
  ...
  // CRITICAL FIX: Remove tempDiv from document after extraction
  document.body.removeChild(tempDiv);

  // 🐛 DEBUG: Log raw extracted styles BEFORE conversion
  console.log('🔍 [ZaloChatView] Raw extracted styles (before conversion):', {
    rawStylesCount: styles.length,
    rawStyles: styles,
    editorHTML: html.substring(0, 200),
    plainText: plainText.substring(0, 100),
    hasListStyles: styles.some(s => s.unorderedList || s.orderedList),
    hasColorStyles: styles.some(s => s.color),
    rawStylesPreview: styles.map(s => ({
      ...s,
      text: plainText.substring(s.start, s.end)
    }))
  });

  // 🔧 FIX: Convert to Zalo API format
  // According to zalo-api-final documentation, send SEPARATE style objects
  // Instead of combining with commas like "b,i,c_db342e", send separate objects:
  // [{ start: 0, len: 10, st: "b" }, { start: 0, len: 10, st: "i" }, { start: 0, len: 10, st: "c_db342e" }]
  // This is ESPECIALLY important for list styles (lst_1, lst_2) which may not work when combined
  // OLD CODE - UNREACHABLE (function already returned above with Quill implementation)
  // const zaloApiStyles = [];
  ...
  /* styles.forEach(style => {
    ...
    // Skip if length is 0 or negative
    if (len <= 0) return;

    // Send SEPARATE style object for each style type
    if (style.bold) {
      zaloApiStyles.push({ start, len, st: 'b' });
    }
    if (style.italic) {
      zaloApiStyles.push({ start, len, st: 'i' });
    }
    if (style.underline) {
      zaloApiStyles.push({ start, len, st: 'u' });
    }
    if (style.strikethrough) {
      zaloApiStyles.push({ start, len, st: 's' });
    }

    // Color code (already in format 'c_XXXXXX')
    if (style.color) {
      zaloApiStyles.push({ start, len, st: style.color });
    }

    // List styles - MUST be sent separately from other styles
    // TextStyle.UnorderedList = "lst_1"
    // TextStyle.OrderedList = "lst_2"
    if (style.unorderedList) {
      zaloApiStyles.push({ start, len, st: 'lst_1' });
    }
    if (style.orderedList) {
      zaloApiStyles.push({ start, len, st: 'lst_2' });
    }
  });

  console.log('📝 [ZaloChatView] Extracted styles (Zalo API format):', {
    stylesCount: zaloApiStyles.length,
    styles: zaloApiStyles,
    plainText: plainText,
    plainTextLength: plainText.length,
    stylesPreview: zaloApiStyles.map(s => ({
      ...
      len: s.len,
      st: s.st,
      text: plainText.substring(s.start, s.start + s.len)
    }))
  }); */

  // OLD CODE END - Return Zalo API formatted styles array if we have any, otherwise return null
  // return zaloApiStyles.length > 0 ? zaloApiStyles : null;
};
...
// Upload video - Upload to Laravel backend
const uploadVideo = async (file, textMessage = '') => {
  ...
  try {
    ...
    // Upload to Laravel backend which will handle zalo-service communication
...
// Upload audio - Upload to Laravel backend
...
// Parse mentions from text (@username or @all)
const parseMentions = (text) => {
  console.log('🔍 [ZaloChatView] parseMentions called', {
    text: text,
    textLength: text?.length || 0,
    itemType: props.itemType,
    isGroup: props.itemType === 'groups',
    groupMembersCount: groupMembers.value.length,
    groupMembersPreview: groupMembers.value.slice(0, 3).map(m => ({
  ...
  if (!text || props.itemType !== 'groups') {
    console.log('⚠️ [ZaloChatView] Skipping parseMentions - not a group or empty text');
    return null;
  }

  const mentions = [];
  
  // Regex to find @mentions: @ followed by word characters or "all"
  const mentionRegex = /@(all|[\w\s]+?)(?=\s|$|[^\w])/gi;
  let match;

  console.log('🔍 [ZaloChatView] Searching for mentions in text:', text);

  while ((match = mentionRegex.exec(text)) !== null) {
    const mentionText = match[1]; // Text after @
    const position = match.index; // Position of @ in text
    
    console.log('🔍 [ZaloChatView] Found potential mention:', {
      fullMatch: match[0],
      mentionText: mentionText,
      position: position,
    });
    ...
    if (mentionText.toLowerCase() === 'all') {
      // @all mention - use special UID "-1"
      mentions.push({
        pos: position,
        len: match[0].length, // Length of @all
        uid: '-1' // Zalo uses "-1" for @all
      });
      console.log('👥 [ZaloChatView] Found @all mention at position:', position);
    } else {
      // Regular user mention - find matching member
      console.log('🔍 [ZaloChatView] Searching for member with name:', mentionText, {
        trimmed: mentionText.trim(),
        lowercase: mentionText.toLowerCase().trim(),
        allMemberNames: groupMembers.value.map(m => ({
          display_name: m.display_name,
          zalo_user_id: m.zalo_user_id,
        })),
      });
      ...
      const member = groupMembers.value.find(m => {
        const memberName = m.display_name || '';
        const matches = memberName.toLowerCase() === mentionText.toLowerCase().trim();
        console.log('🔍 [ZaloChatView] Comparing:', {
          memberName: memberName,
          memberNameLower: memberName.toLowerCase(),
          mentionText: mentionText,
          mentionTextLower: mentionText.toLowerCase().trim(),
          matches: matches,
        });
        return matches;
      });

      // Get user ID from member (could be 'id', 'zalo_user_id', or 'uid')
      const userId = member?.id || member?.zalo_user_id || member?.uid;
      
      console.log('🔍 [ZaloChatView] Found member result:', {
        member: member,
        hasMember: !!member,
        id: member?.id,
      ...
      if (member && userId) {
        mentions.push({
          pos: position,
          len: match[0].length, // Length of @username
          uid: userId
        });
        console.log('👤 [ZaloChatView] Found user mention:', member.display_name, 'UID:', userId, 'at position:', position);
      } else {
        console.log('⚠️ [ZaloChatView] No matching member found for:', mentionText, {
          member: member,
          memberKeys: member ? Object.keys(member) : null,
          groupMembersCount: groupMembers.value.length,
          firstMember: groupMembers.value[0],
          allMembers: groupMembers.value,
        });
      }
    }
  }

  console.log('✅ [ZaloChatView] parseMentions result:', {
...
const sendMessage = async () => {
  ...
  if (showEditor.value && editorContent.value && text) {
    ...
    window.__DEBUG_ZALO_EXTRACT__ = true;
    
    console.log('🚀 [ZaloChatView] ===== SENDING MESSAGE - EXTRACTING STYLES =====');
    styles = extractStylesFromEditor();
    
    // Disable debug logging
    window.__DEBUG_ZALO_EXTRACT__ = false;
    
    console.log('📤 [ZaloChatView] Extracted styles for sending:', {
      styles: styles,
      textLength: text.length,
      textPreview: text.substring(0, 50),
      hasStyles: styles && styles.length > 0,
      stylesCount: styles?.length || 0,
      // Check for list styles in the converted format (check 'st' field)
      hasUnorderedList: styles?.some(s => s.st && s.st.includes('lst_1')),
      hasOrderedList: styles?.some(s => s.st && s.st.includes('lst_2')),
      // Check for color styles
      hasColorStyles: styles?.some(s => s.st && s.st.includes('c_')),
      // Show all st values
      allStValues: styles?.map(s => s.st) || []
    });
  }
...
const sendTextMessage = async (text, styles = null) => {
  ...
  try {
    ...
    // Parse and add mentions if any
    const mentions = parseMentions(text);
    if (mentions && mentions.length > 0) {
      payload.mentions = mentions;
      console.log('👥 [ZaloChatView] Adding mentions to payload:', mentions);
    }
...
const createEvent = async () => {
  ...
  try {
    ...
    // Use Laravel API which handles authentication and permissions
    const response = await axios.post(
      '/api/zalo/messages/create-reminder',
      {
        account_id: accountId,
        recipient_id: recipientId,
        recipient_type: props.itemType === 'groups' ? 'group' : 'user',
        thread_id: recipientId, // Required by backend
...
// Example: "mr.mike-user123132@songthuy.edu.vn" → "mr.mike-user123132"
...
// Show reaction users
const showReactionUsers = async (messageId, reaction) => {
  try {
    // Load full reaction details with user info
    ...
    if (!message || !message.reactions || message.reactions.length === 0) {
      console.warn('No reactions found for message:', messageId);
      return;
    }
    
    // Set current reactions and show modal
    currentReactions.value = message.reactions;
    ...
    Swal.fire({
      icon: 'error',
      title: t('common.error') || 'Error',
      text: 'Không thể tải thông tin biểu cảm',
      confirmButtonText: t('common.ok') || 'OK',
    });
  }
};
...
watch(() => props.item, (newItem, oldItem) => {
  ...
  if (newItem?.id && currentAccountId.value) {
    ...
    // Load group members for mentions (if it's a group)
    loadGroupMembers();
...
const initializeQuill = () => {
  if (!editorContent.value || quillInstance.value) {
    console.log('⚠️ [ZaloChatView] Cannot initialize Quill:', {
      editorContentExists: !!editorContent.value,
      quillAlreadyInitialized: !!quillInstance.value
    });
    return;
  }

  console.log('🔵 [ZaloChatView] Initializing Quill editor...');

  try {
    editorContent.value.innerHTML = '';

    // SIMPLE: Use Quill's built-in toolbar
    // IMPORTANT: Only show colors that Zalo supports
    quillInstance.value = new Quill(editorContent.value, {
      theme: 'snow',
      modules: {
        toolbar: [
          ['bold', 'italic', 'underline'],
          [{ 'color': ['#db342e', '#f27806', '#f7b503', '#15a85f'] }], // Red, Orange, Yellow, Green
          [{ 'list': 'bullet' }]
        ]
      },
      placeholder: t('zalo.type_message') || 'Nhập tin nhắn...',
    });

    // Only sync text content and detect mentions
    quillInstance.value.on('text-change', () => {
      const text = quillInstance.value.getText();
      messageText.value = text.endsWith('\n') ? text.slice(0, -1) : text;
      
      // Detect @ for mentions in Quill Editor
      handleQuillMentionDetection();
    });

    // Enter on empty list item removes list formatting
    quillInstance.value.keyboard.addBinding({
      key: 'Enter',
    }, (range) => {
      // Check if we're in a list
      const format = quillInstance.value.getFormat(range.index);
      if (format.list) {
        // Check if current line is empty
        const [line] = quillInstance.value.getLine(range.index);
        const lineText = line.domNode.textContent;

        if (!lineText || lineText.trim() === '') {
          // Empty list item - remove list formatting
          quillInstance.value.format('list', false);
    ...
    quillInstance.value.keyboard.addBinding({
      ...
      return true; // Allow default ESC behavior
    });

    quillReady.value = true;
    console.log('✅ Quill initialized with built-in toolbar');
  } catch (error) {
...
// Watch showEditor - destroy and recreate Quill every time
watch(showEditor, async (newValue, oldValue) => {
  console.log('🔵 [watch(showEditor)] Fired!', {
    oldValue,
    newValue,
    quillInstance: !!quillInstance.value,
    quillReady: quillReady.value
  });

  if (newValue) {
    // Editor is being opened - create fresh Quill instance
    console.log('📝 [ZaloChatView] Editor opening, will initialize Quill after DOM renders...');

    // Wait for v-if to create the DOM element
    await nextTick();
    await nextTick(); // Extra wait to ensure DOM is fully ready

    initializeQuill();
  } else {
    // Editor is being closed - destroy Quill instance
    console.log('🗑️ [watch(showEditor)] Editor closing, destroying Quill instance...');

    if (quillInstance.value) {
      try {
        // Quill doesn't have a built-in destroy method, but we can clean up
        quillInstance.value.off('text-change'); // Remove event listeners
        quillInstance.value = null;
        console.log('✅ [watch(showEditor)] Quill instance destroyed');
      } catch (err) {
        console.error('❌ Error destroying Quill:', err);
      }
    }
...
// Handle images sent from ZaloImageUploader component (must be top-level for template access)
const handleImagesSent = async (result) => {
  console.log('[ZaloChatView] Images uploaded:', result);
  console.log('[ZaloChatView] DEBUG props.item:', props.item);
  console.log('[ZaloChatView] DEBUG props.itemType:', props.itemType);
  
  // Determine correct recipient_id field
  const recipientId = props.itemType === 'groups' 
    ? (props.item.zalo_group_id || props.item.id)
    : (props.item.zalo_user_id || props.item.id);
  
  console.log('[ZaloChatView] DEBUG recipientId:', recipientId);
  console.log('[ZaloChatView] DEBUG currentAccountId:', currentAccountId.value);
  
  try {
    // Send each uploaded image as separate message via Zalo
    for (const image of result.uploaded) {
      const payload = {
        account_id: currentAccountId.value,
        recipient_id: recipientId,
        recipient_type: props.itemType === 'groups' ? 'group' : 'user',
        content_type: 'image',
        media_path: image.absolute_path,
        message: '',
      };
      
      console.log('[ZaloChatView] Sending image with payload:', payload);
      
      await axios.post('/api/zalo/messages/send', payload);
    }
    ...
    if (result.successCount > 0) {
      useSwal().fire({
        ...
        text: `Đã gửi ${result.successCount} ảnh`,
        timer: 2000,
        showConfirmButton: false
      });
    }
    ...
    if (result.errors.length > 0) {
      useSwal().fire({
        icon: 'warning',
        title: 'Một số ảnh thất bại',
        text: `${result.errors.length}/${result.totalCount} ảnh không thể gửi`,
        timer: 3000,
        showConfirmButton: false
      });
    }
    ...
    console.error('[ZaloChatView] Error sending images:', error);
    useSwal().fire({
      icon: 'error',
      title: 'Lỗi',
      text: error.response?.data?.message || 'Không thể gửi ảnh',
    });
  }
};
...
onMounted(() => {
  ...
  // Don't initialize Quill here - wait for editor to be shown
  // (Quill can't initialize selection module on hidden elements)
  ...
  try {
    console.log('🔧 [ZaloChatView] Setting up message:recalled listener...', {
      hasSocket: !!zaloSocket,
      hasOnFunction: zaloSocket && typeof zaloSocket.on === 'function',
      currentAccountId: currentAccountId.value,
      conversationId: props.item?.id
    });

    if (zaloSocket && typeof zaloSocket.on === 'function') {
      unsubscribeRecalled = zaloSocket.on('message:recalled', (data) => {
        console.log('🔙 [ZaloChatView] Message recalled event received', {
          zalo_message_id: data.message_id,
          cli_msg_id: data.cli_msg_id,
          thread_id: data.thread_id,
          account_match: data.account_id === currentAccountId.value,
          thread_match: data.thread_id === props.item?.id,
        });

        if (data.account_id === currentAccountId.value &&
            data.thread_id === props.item?.id) {
          // Find message by Zalo message_id (not database ID!)
          const message = messages.value.find(m =>
            m.message_id === data.message_id ||
            m.metadata?.cliMsgId === data.cli_msg_id ||
            m.metadata?.cli_msg_id === data.cli_msg_id
          );
          ...
          if (message) {
            ...
            if (messagesCache.has(conversationId)) {
              const cachedMessages = messagesCache.get(conversationId);
              const cachedMessage = cachedMessages.find(m =>
                m.message_id === data.message_id ||
                m.metadata?.cliMsgId === data.cli_msg_id
              );
              if (cachedMessage) {
      ...
      console.log('✅ [ZaloChatView] message:recalled listener registered successfully');
    } else {
      console.warn('⚠️ [ZaloChatView] Cannot register message:recalled listener - socket not available');
    }
  } catch (error) {
...
/* Quill editor styling - keep it simple */
:deep(.ql-container) {
  font-size: 14px;
}
