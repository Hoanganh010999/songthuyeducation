...
const save = async () => {
  ...
  try {
    ...
    const data = {
      ...
      total_sessions: form.value.total_sessions || 30 // Ensure total_sessions is always sent
    };
...
onMounted(async () => {
  await loadData(); // Wait for syllabuses to load first
  ...
  if (props.classData) {
    ...
    // If class has a syllabus, get total_sessions from that syllabus
    if (form.value.lesson_plan_id) {
      const selectedSyllabus = syllabuses.value.find(s => s.id === parseInt(form.value.lesson_plan_id));
      if (selectedSyllabus) {
        form.value.total_sessions = selectedSyllabus.total_sessions;
        console.log('✅ [ClassForm] Updated total_sessions from syllabus:', selectedSyllabus.total_sessions);
      }
    }
  }
});
