...
const loadStudents = async (page = 1) => {
    try {
        ...
        const res = await api.get('/quality/students', { params });
...
const showAddToClassModal = async (student) => {
    try {
        ...
        const classesRes = await api.get('/quality/classes', {
        ...
        if (result.isConfirmed) {
            await api.post(`/quality/classes/${result.value}/students`, {
...
const toggleStudentStatus = async (student) => {
    try {
        ...
        if (result.isConfirmed) {
            await api.patch(`/quality/students/${student.id}/status`, {
                is_active: newStatus
            });
