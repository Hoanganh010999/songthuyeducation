<template>
  <div class="space-y-4">
    ...
    <!-- Employee List - Desktop Table View (hidden on mobile) -->
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr v-else v-for="employee in employees" :key="employee?.id || Math.random()" class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3">
              ...
              <div class="text-xs text-gray-500 mt-1">{{ employee?.phone || '-' }}</div>
            </td>
            <td class="px-4 py-3 text-center">
              ...
              >
                {{ employee?.employment_status === 'active' ? 'Đang làm' : 'Nghỉ việc' }}
              </span>
            </td>
    ...
    <!-- Employee List - Mobile Card View (visible on mobile) -->
    <div class="md:hidden space-y-3">
      <div v-if="loading" class="bg-white rounded-lg border border-gray-200 p-6 text-center text-gray-500">
        Đang tải...
      </div>
      <div v-else-if="employees.length === 0" class="bg-white rounded-lg border border-gray-200 p-6 text-center text-gray-500">
        Không có nhân viên nào
      </div>
      <div
        v-else
      ...
      >
        ...
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center">
            <div class="flex-shrink-0">
              ...
              <div
                v-else
                class="h-12 w-12 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold"
              >
                {{ getInitials(employee?.name) }}
              </div>
            </div>
          ...
          >
            {{ employee?.employment_status === 'active' ? 'Đang làm' : 'Nghỉ' }}
          </span>
        </div>
        ...
        <div class="space-y-2 mb-3">
          <div>
            <div class="text-xs text-gray-500">Phòng ban</div>
            <div class="text-sm text-gray-900 font-medium">
              {{ (employee?.departments && Array.isArray(employee.departments)) ? employee.departments.map(d => d?.name).filter(Boolean).join(', ') : '-' }}
            </div>
          </div>
        ...
        <div class="flex space-x-2 pt-3 border-t border-gray-100">
          <button
            @click="viewEmployee(employee)"
            class="flex-1 px-3 py-2 text-sm text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition"
          >
            Chi tiết
          </button>
          <button
            v-if="authStore.hasPermission('employees.manage')"
            @click="editEmployee(employee)"
            class="flex-1 px-3 py-2 text-sm text-indigo-600 bg-indigo-50 rounded-md hover:bg-indigo-100 transition"
          >
            Sửa
          </button>
        </div>
