    ...
    <div>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <select v-model="filters.type" @change="loadVouchers" 
                        class="px-4 py-2 border border-gray-300 rounded-lg">
                ...
                <input v-model="filters.code" @input="loadVouchers" type="text" 
                       :placeholder="t('vouchers.code')"
                       class="px-4 py-2 border border-gray-300 rounded-lg">
                    ...
                    <tr v-for="voucher in vouchers" :key="voucher.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                  :class="voucher.type === 'percentage' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div>{{ voucher.usage_count || 0 }} / {{ voucher.usage_limit || '∞' }}</div>
                            <div class="text-xs text-gray-400">{{ voucher.usage_per_customer }}x / {{ t('vouchers.per_customer') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div v-if="voucher.start_date">{{ formatDate(voucher.start_date) }}</div>
                            <div v-if="voucher.end_date">→ {{ formatDate(voucher.end_date) }}</div>
                            <div v-if="!voucher.start_date && !voucher.end_date" class="text-gray-400">{{ t('vouchers.no_expiry') }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
