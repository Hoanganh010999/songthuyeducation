<template>
          ...
          <div v-if="activeComposerTab === 'homework'" class="space-y-3">
            ...
            <div v-if="homeworkForm.session_id" class="bg-orange-50 rounded-lg p-3">
              ...
              <div v-else-if="sessionFiles.length === 0" class="text-xs text-gray-500 text-center py-2">
                Chưa có homework nào cho buổi học này
              </div>
              <div v-else class="grid grid-cols-1 gap-2 max-h-32 overflow-y-auto">
                <div
                  v-for="file in sessionFiles"
                  :key="file.id"
                  class="flex items-center space-x-2 p-2 bg-white hover:bg-orange-100 rounded border border-orange-200 transition-colors"
                >
                  <input
                    type="checkbox"
                    :value="file.id"
                    v-model="homeworkForm.file_ids"
                    class="w-3.5 h-3.5 text-orange-600 border-gray-300 rounded focus:ring-orange-500 cursor-pointer"
                  />
                  <span class="text-base flex-shrink-0">📝</span>
                  <div class="flex-1 min-w-0 flex items-center justify-between">
                    <p class="text-xs font-medium text-gray-800 truncate">{{ file.title }}</p>
                    <button
                      @click.stop="openHomeworkPreview(file)"
                      class="ml-2 text-blue-600 hover:text-blue-800 flex items-center space-x-1 text-xs"
                      type="button"
                    >
                ...
                <div v-else-if="homeworkSubmissions[post.metadata.homework_id] && homeworkSubmissions[post.metadata.homework_id].length > 0" class="space-y-1.5">
                  <div v-for="submission in homeworkSubmissions[post.metadata.homework_id]" :key="submission.id" 
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        @click="openGradingModal(submission, post.metadata.homework_id)"
                        class="text-sm text-gray-700 hover:text-orange-600 hover:underline font-medium transition-colors"
                      >
                        {{ submission.student?.name || t('common.unknown') }}
                      </button>
                    </div>
          ...
          <div v-for="homework in upcomingHomework" :key="homework.id" 
            ...
            <div class="flex items-start gap-2">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="authStore.isStudent || authStore.isParent" class="mt-3 pt-3 border-t border-orange-200">
                  ...
                  <div v-if="homework.is_parent && homework.children_count > 0" class="space-y-3">
                    ...
                    >
                      ...
                      <!-- Not Submitted - Show Do Homework Button -->
                      <div v-else>
                        <button
                          @click="openHomeworkModalForChild(homework, child)"
                          class="w-full px-3 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 transition-colors flex items-center justify-center gap-2"
                        >
                          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div v-else>
                    <button
                      @click="openHomeworkModal(homework)"
                      class="w-full px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 transition-colors flex items-center justify-center gap-2"
                    >
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
  ...
  <HomeworkAssignmentPreviewModal
    v-if="showHomeworkPreview && selectedHomeworkForPreview"
    :homework="selectedHomeworkForPreview"
    @close="showHomeworkPreview = false; selectedHomeworkForPreview = null"
  />

  <!-- Homework Do Modal (For Students) -->
  <HomeworkDoModal
    v-if="showHomeworkDoModal && selectedHomeworkForDo"
    :homework="selectedHomeworkForDo"
    :studentId="selectedStudentIdForDo"
    @close="closeHomeworkDoModal"
    @submitted="onHomeworkSubmitted"
  />

  <!-- Homework Grading Modal (For Teachers and Student Review) -->
  <HomeworkGradingModal
    v-if="showGradingModal && selectedSubmissionForGrading"
    :show="showGradingModal"
    :submission="selectedSubmissionForGrading"
    :homeworkId="currentHomeworkIdForGrading"
    :mode="gradingMode"
    @close="closeGradingModal"
    @graded="onHomeworkGraded"
  />
...
import HomeworkAssignmentPreviewModal from '../../components/homework/HomeworkAssignmentPreviewModal.vue';
import HomeworkDoModal from '../../components/homework/HomeworkDoModal.vue';
...
// Computed property for Zalo account ID (use class's account or fallback to primary)
const zaloAccountIdToUse = computed(() => {
  return selectedClass.value?.zalo_account_id || primaryZaloAccountId.value || null;
});
...
const primaryZaloAccountId = ref(null);
...
const homeworkEditor = useQuillEditor({
  placeholder: t('course.homework_description'),
  toolbar: [
    ['bold', 'italic', 'underline'],
    [{ 'color': ['#db342e', '#f27806', '#f7b503', '#15a85f'] }],  // Red, Orange, Yellow, Green (Zalo colors)
    [{ 'list': 'bullet' }]
  ]
});
...
const showHomeworkPreview = ref(false);
const selectedHomeworkForPreview = ref(null);

// Homework Do Modal (for students/parents)
const showHomeworkDoModal = ref(false);
const selectedHomeworkForDo = ref(null);
const selectedStudentIdForDo = ref(null);
const selectedSubmissionForGrading = ref(null);
const showGradingModal = ref(false);
const currentHomeworkIdForGrading = ref(null);
const gradingMode = ref('grading'); // 'grading' or 'review'
...
const loadClassStudents = async () => {
  ...
  try {
    ...
    // Only teachers/admins can access this endpoint, students don't have permission
    let allStudents = [];
    if (authStore.isTeacher || authStore.isAdmin || authStore.isSuperAdmin) {
      try {
        ...
        console.log('[ClassroomBoard] ⚠️ Cannot load all students (permission denied), continuing with class students only');
      }
    } else {
      console.log('[ClassroomBoard] ℹ️ User is student, skipping all students load');
    }
...
// Load primary Zalo account (used when class doesn't have account set)
const loadPrimaryZaloAccount = async () => {
  try {
    ...
    if (response.data.success && response.data.data.length > 0) {
      ...
      if (primaryAccount) {
        primaryZaloAccountId.value = primaryAccount.id;
        console.log('[ClassroomBoard] ✅ Primary Zalo account loaded:', primaryZaloAccountId.value);
      } else {
        console.warn('[ClassroomBoard] ⚠️ No primary Zalo account found');
      }
    } else {
      console.warn('[ClassroomBoard] ⚠️ No Zalo accounts found');
    }
  } catch (error) {
    console.error('[ClassroomBoard] ❌ Error loading primary Zalo account:', error);
  }
};
...
const clearGroupFromClass = async () => {
  ...
  try {
    const response = await axios.patch(`/api/quality/classes/${selectedClass.value.id}/zalo-group`, {
    ...
    if (response.data.success) {
      ...
      selectedClass.value.zalo_account_id = null;
      selectedClass.value.zalo_group_id = null;
      selectedClass.value.zalo_group_name = null;
      console.log('[ClassroomBoard] ✅ Cleared invalid group from class');
    }
  } catch (error) {
    console.error('[ClassroomBoard] ❌ Error clearing group from class:', error);
  }
};
...
const validateGroup = async () => {
  ...
  // Use zaloAccountIdToUse which falls back to primary if class doesn't have one
  const accountId = zaloAccountIdToUse.value;
  if (!accountId) {
    console.warn('[ClassroomBoard] ⚠️ No Zalo account available for validation');
    return;
  }
  ...
  try {
    ...
    if (response.data.success) {
      if (response.data.exists) {
        ...
        // Group doesn't exist - clear it automatically
        console.warn('[ClassroomBoard] Group not found in Zalo, clearing from class...');
...
const onClassChange = async () => {
  ...
  // Reset homework form when switching classes
  homeworkForm.value = {
    title: '',
    description: '',
    session_id: '',
  ...
  // Only teachers/admins need this detailed information, students don't have permission
  if (classId.value && (authStore.isTeacher || authStore.isAdmin || authStore.isSuperAdmin)) {
    try {
      ...
      if (response.data.success) {
        ...
        // Load primary Zalo account if class doesn't have one
        if (!selectedClass.value.zalo_account_id) {
          console.log('[ClassroomBoard] ⚠️ Class has no Zalo account, loading primary...');
          await loadPrimaryZaloAccount();
        }
    ...
    console.log('[ClassroomBoard] ℹ️ User is student, skipping detailed class data load');
    // For students, create a minimal selectedClass object from availableClasses
    const classInfo = availableClasses.value.find(c => c.id === classId.value);
    if (classInfo) {
...
const onSessionChange = async () => {
  ...
  // Load homework assignments for the selected session
...
const loadSessionFiles = async () => {
  ...
  try {
    // Load homework assignments from syllabus management instead of Google Drive files
    const response = await axios.get('/api/course/homework/assignments', {
      params: {
        session_id: homeworkForm.value.session_id
      }
    });
    if (response.data.success) {
    ...
    console.error('Error loading session homework assignments:', error);
    showError(t('common.error_occurred'));
  } finally {
...
const openHomeworkPreview = async (homework) => {
  try {
    console.log('[ClassroomBoard] 🔍 openHomeworkPreview called with:', homework);
    console.log('[ClassroomBoard] 📦 homework.id:', homework.id);
    console.log('[ClassroomBoard] 🌐 API URL:', `/api/homework/bank/${homework.id}`);
    
    // Load full homework details with exercises from homework bank
    const response = await axios.get(`/api/homework/bank/${homework.id}`, {
      params: {
        include: 'exercises.options,creator,session',
        branch_id: authStore.user.branch_id
      }
    });

    console.log('[ClassroomBoard] ✅ API response:', response.data);

    if (response.data.success) {
      selectedHomeworkForPreview.value = response.data.data;
      showHomeworkPreview.value = true;
      console.log('[ClassroomBoard] ✅ Preview modal opened');
    } else {
      console.warn('[ClassroomBoard] ⚠️ API returned success=false');
    }
  } catch (error) {
    console.error('[ClassroomBoard] ❌ Error loading homework details:', error);
    console.error('[ClassroomBoard] ❌ Error response:', error.response?.data);
    showError('Không thể tải thông tin homework');
  }
};

// Open homework modal for students to do homework or review submitted homework
const openHomeworkModal = async (homework) => {
  try {
    console.log('[ClassroomBoard] 🔍 openHomeworkModal called');
    console.log('[ClassroomBoard] 📦 homework object:', homework);
    console.log('[ClassroomBoard] 📊 submission_status:', homework.submission_status);
    console.log('[ClassroomBoard] 🆔 submission_id:', homework.submission_id);

    // Check if homework is already submitted/graded
    const isSubmitted = homework.submission_status === 'submitted' || homework.submission_status === 'graded';

    console.log('[ClassroomBoard] ✅ isSubmitted:', isSubmitted);

    if (isSubmitted) {
      console.log('[ClassroomBoard] 🎯 Fetching student submission for homework:', homework.id);
      // Student wants to review their submitted homework
      // Fetch student's own submission via API
      const submissionResponse = await axios.get(`/api/course/homework/${homework.id}/my-submission`);

      console.log('[ClassroomBoard] 📦 Submission response:', submissionResponse.data);

      if (submissionResponse.data.success && submissionResponse.data.data) {
        console.log('[ClassroomBoard] ✅ Opening REVIEW modal with submission:', submissionResponse.data.data.id);
        selectedSubmissionForGrading.value = submissionResponse.data.data;
        currentHomeworkIdForGrading.value = homework.id;
        gradingMode.value = 'review'; // Set to review mode (read-only)
        showGradingModal.value = true;
      } else {
        console.log('[ClassroomBoard] ⚠️ No submission found, opening DO modal');
        // No submission found (edge case) - fall back to DO modal
        const response = await axios.get(`/api/course/homework/assignments/${homework.id}`, {
          params: {
            include: 'exercises.options'
          }
        });

        if (response.data.success) {
          selectedHomeworkForDo.value = response.data.data;
          selectedStudentIdForDo.value = null;
          showHomeworkDoModal.value = true;
        }
      }
    } else {
      console.log('[ClassroomBoard] 📝 Opening DO modal (not submitted)');
      // Student wants to do homework (not submitted yet)
      // Load full homework details with exercises
      const response = await axios.get(`/api/course/homework/assignments/${homework.id}`, {
        params: {
          include: 'exercises.options'
        }
      ...
      if (response.data.success) {
        selectedHomeworkForDo.value = response.data.data;
        selectedStudentIdForDo.value = null; // Student's own homework
        showHomeworkDoModal.value = true;
      }
    }
  } catch (error) {
...
// Open homework modal for parent doing homework for a child
const openHomeworkModalForChild = async (homework, child) => {
  try {
    // Load full homework details with exercises
    const response = await axios.get(`/api/course/homework/assignments/${homework.id}`, {
      params: {
    ...
    if (response.data.success) {
      selectedHomeworkForDo.value = response.data.data;
      selectedStudentIdForDo.value = child.user_id; // Parent doing homework for child
      showHomeworkDoModal.value = true;
    }
  } catch (error) {
    console.error('Error loading homework details:', error);
...
// Open homework grading modal (teacher/admin view)
const openGradingModal = (submission, homeworkId) => {
  console.log('[ClassroomBoard] 🎯 openGradingModal called');
  console.log('[ClassroomBoard] 📦 submission:', submission);
  console.log('[ClassroomBoard] 🆔 submission.id:', submission?.id);
  console.log('[ClassroomBoard] 📚 homeworkId:', homeworkId);

  selectedSubmissionForGrading.value = submission;
  currentHomeworkIdForGrading.value = homeworkId;
  gradingMode.value = 'grading'; // Set to grading mode for teachers
  showGradingModal.value = true;

  console.log('[ClassroomBoard] ✅ After setting values:');
  console.log('[ClassroomBoard]    - showGradingModal.value:', showGradingModal.value);
  console.log('[ClassroomBoard]    - selectedSubmissionForGrading.value:', selectedSubmissionForGrading.value);
  console.log('[ClassroomBoard]    - currentHomeworkIdForGrading.value:', currentHomeworkIdForGrading.value);
  console.log('[ClassroomBoard]    - v-if will be:', showGradingModal.value && !!selectedSubmissionForGrading.value);
};

// Close grading modal
const closeGradingModal = () => {
  showGradingModal.value = false;
  selectedSubmissionForGrading.value = null;
  currentHomeworkIdForGrading.value = null;
};

// Handle homework graded
const onHomeworkGraded = async (gradedSubmission) => {
  // Refresh submissions list
  if (currentHomeworkIdForGrading.value) {
    await loadSubmissions(currentHomeworkIdForGrading.value);
  }

  showSuccess('Đã lưu kết quả chấm bài');
};

// Close homework do modal
const closeHomeworkDoModal = () => {
  showHomeworkDoModal.value = false;
  selectedHomeworkForDo.value = null;
  selectedStudentIdForDo.value = null;
};

// Handle homework submission complete
const onHomeworkSubmitted = async (response) => {
  closeHomeworkDoModal();

  // Show success message
  showSuccess('Đã nộp bài tập thành công!');

  // Reload homework list to update submission status
  await loadUpcomingHomework();
  await loadPosts();
};

const sendHomeworkToZaloGroup = async (homeworks) => {
  try {
    console.log('📅 Creating Zalo reminder and sending message for homework:', homeworks);

    // Build reminder title
    let reminderTitle = '';

    if (homeworks.length === 1) {
      // Single homework - use homework title
      reminderTitle = `📚 Bài tập: ${homeworks[0].title}`;
    } else {
      // Multiple homework - show count and class name
      reminderTitle = `📚 ${homeworks.length} bài tập mới - ${selectedClass.value.name}`;
    }

    // Find the earliest deadline among all homework
    ...
    // If no deadline found, use tomorrow as default
    if (!earliestDeadline) {
      earliestDeadline = new Date();
      earliestDeadline.setDate(earliestDeadline.getDate() + 1);
      earliestDeadline.setHours(17, 0, 0, 0); // Set to 5 PM tomorrow
    }

    // Convert to timestamp (milliseconds)
    const startTime = earliestDeadline.getTime();

    // 1. Create Zalo reminder/event (optional - don't block message sending if fails)
    try {
      console.log('📅 Creating reminder with:', {
        title: reminderTitle,
        start_time: startTime,
        deadline_date: earliestDeadline.toISOString(),
      });

      const reminderResponse = await axios.post('/api/zalo/messages/create-reminder', {
        account_id: selectedClass.value.zalo_account_id,
        thread_id: selectedClass.value.zalo_group_id,
        type: 'group',
        title: reminderTitle,
        start_time: startTime,
        emoji: '📚',
      ...
      if (reminderResponse.data.success) {
        console.log('✅ Zalo reminder created successfully:', reminderResponse.data);
      } else {
        console.error('❌ Failed to create Zalo reminder:', reminderResponse.data);
      }
    } catch (reminderError) {
      console.warn('⚠️ Failed to create reminder (continuing with message):', reminderError.response?.data || reminderError.message);
      // Continue to send message even if reminder fails
    }

    // 2. Send formatted text message with post content from Quill editor
    const styles = [];
    let message = '';
    let pos = 0;

    // Title - Bold + Orange color (using correct st format)
    const title = '📚 THÔNG BÁO BÀI TẬP MỚI 📚';
    message += title + '\n\n';
    styles.push({
      start: pos,
      len: title.length,
    ...
    // Homework count
    const countLabel = 'Số lượng: ';
    message += countLabel;
    pos += countLabel.length;

    const count = `${homeworks.length} bài tập`;
    message += count + '\n\n';
    styles.push({
      start: pos,
      len: count.length,
    ...
    homeworks.forEach((hw, index) => {
      ...
      if (hw.deadline) {
        const deadline = new Date(hw.deadline);
        const deadlineText = `   📅 Hạn nộp: ${deadline.toLocaleDateString('vi-VN')} ${deadline.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}\n`;
        message += deadlineText;
        pos += deadlineText.length;
      }

      // Get editor content with styles (using new extractStyles method)
      if (homeworkEditor?.getText && homeworkEditor?.extractStyles) {
        const editorText = homeworkEditor.getText();

        if (editorText && editorText.trim()) {
          // Add a description label
          const descLabel = '   📝 Nội dung: ';
          message += descLabel;
          pos += descLabel.length;

          const contentStartPos = pos;
          message += editorText + '\n';

          // Extract styles from Quill Delta
          const editorStyles = homeworkEditor.extractStyles();
          console.log('📝 [sendHomeworkToZaloGroup] Editor styles:', editorStyles);

          if (editorStyles && editorStyles.length > 0) {
            // Adjust style positions to account for the text we added before
            editorStyles.forEach(style => {
              styles.push({
                start: contentStartPos + style.start,
                len: style.len,
                st: style.st
              });
            });
    ...
    // Footer
    const footer = '👉 Vui lòng kiểm tra Classroom Board để xem chi tiết và nộp bài!';
    message += '\n' + footer;
    pos++;  // for \n
    styles.push({
      start: pos,
    ...
    console.log('💬 Sending formatted message to Zalo group');
    console.log('📝 Message content:', message);
    console.log('🎨 Styles array:', JSON.stringify(styles, null, 2));

    // Send formatted text message
    const messageResponse = await axios.post('/api/zalo/messages/send', {
      account_id: selectedClass.value.zalo_account_id,
      recipient_id: selectedClass.value.zalo_group_id,
    ...
    if (messageResponse.data.success) {
      console.log('✅ Zalo message sent successfully:', messageResponse.data);
    } else {
      console.error('❌ Failed to send Zalo message:', messageResponse.data);
    }

    console.log('✅ Completed: Reminder created and message sent to Zalo group');
  } catch (error) {
    console.error('❌ Error sending to Zalo group:', error);
    console.error('Error details:', error.response?.data || error.message);
    // Don't throw error, just log it - we don't want to fail the homework creation if Zalo fails
  }
};
...
const createHomework = async () => {
  ...
  if (homeworkForm.value.file_ids.length === 0) {
    showError('Vui lòng chọn ít nhất một homework');
    return;
  }

  creatingHomework.value = true;
  try {
    // Create posts for each selected homework assignment
    const selectedHomeworks = sessionFiles.value.filter(hw =>
      homeworkForm.value.file_ids.includes(hw.id)
    );

    for (const homework of selectedHomeworks) {
      const payload = {
        class_id: classId.value,
        content: homeworkEditor.getContent() || homeworkForm.value.description || `${homeworkForm.value.title}\n\n${homework.description || ''}`,
        post_type: 'homework',
        metadata: {
          homework_id: homework.id,
          homework_title: homework.title,
          custom_title: homeworkForm.value.title,
          custom_deadline: homeworkForm.value.deadline,
          due_date: homeworkForm.value.deadline || homework.deadline,
          assigned_to: assignmentType.value === 'all' ? null : homeworkForm.value.assigned_to,
          session_info: homework.session ? `Buổi ${homework.session.session_number}: ${homework.session.lesson_title}` : null
        }
      };

      await axios.post(`/api/course/classes/${classId.value}/posts`, payload);
    }

    // Send event message to Zalo group if class has Zalo group configured
    if (selectedClass.value?.zalo_group_id && selectedClass.value?.zalo_account_id) {
      // Add deadline from form to homework objects before sending to Zalo
      const homeworksWithDeadline = selectedHomeworks.map(hw => ({
        ...hw,
        deadline: homeworkForm.value.deadline || hw.deadline,
        title: homeworkForm.value.title || hw.title  // Also use custom title if provided
      }));

      await sendHomeworkToZaloGroup(homeworksWithDeadline);
    }

    showSuccess(`Đã đăng ${selectedHomeworks.length} homework lên classroom board`);
    resetCurrentForm();
    activeComposerTab.value = 'post';
    await loadPosts();
  } catch (error) {
