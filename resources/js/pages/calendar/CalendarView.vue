<template>
  <div class="min-h-screen bg-gray-50 p-6">
    <!-- 🔥 REMOVED: No longer hiding default buttons, we'll use them instead -->
    <!-- Header -->
...
const initCalendar = () => {
  ...
  calendarInstance = new Calendar(calendarContainer.value, {
    ...
    template: {
      ...
      // 🔥 NEW: Use default Edit/Delete buttons for all events (including placement_test)
      // We'll override their click handlers instead of hiding them
      popupDetailButtons(event) {
        // Return undefined to use default buttons
        return undefined;
      },
      time(event) {
  ...
  // 🔥 NEW: Override default Edit/Delete button handlers for placement_test
  calendarInstance.on('beforeUpdateEvent', async (updateData) => {
    try {
      // Toast UI Calendar passes event directly or in updateData.event
      const event = updateData?.event || updateData;
      if (!event || !event.calendarId) {
        return; // Let default handler work
      }
      
      if (event.calendarId === 'placement_test') {
        // Prevent default update, use our custom handler instead
        const foundEvent = allEvents.value.find(ev => ev.id == event.id);
        if (foundEvent) {
          // Await the async function to prevent promise rejection
          await handleEditPlacementTest(foundEvent);
        }
        return false; // Cancel default update
      }
    } catch (error) {
      console.error('[CalendarView] Error in beforeUpdateEvent:', error);
      // Let default handler work on error
      return; // Don't cancel default update on error
    }
  });

  calendarInstance.on('beforeDeleteEvent', async (deleteData) => {
    try {
      // Toast UI Calendar passes event directly or in deleteData.event
      const event = deleteData?.event || deleteData;
      if (!event || !event.calendarId) {
        return; // Let default handler work
      }
      
      if (event.calendarId === 'placement_test') {
        // Prevent default delete, use our custom handler instead
        const foundEvent = allEvents.value.find(ev => ev.id == event.id);
        if (foundEvent) {
          // Await the async function to prevent promise rejection
          await handleDeletePlacementTest(foundEvent);
        }
        return false; // Cancel default delete
      }
    } catch (error) {
      console.error('[CalendarView] Error in beforeDeleteEvent:', error);
      // Let default handler work on error
      return; // Don't cancel default delete on error
    }
  });
...
const getCustomPopupDetailBody = (event) => {
  ...
  if (event.calendarId === 'placement_test') {
    ...
    // 🔥 REMOVED: No longer adding custom Edit/Delete buttons
    // We'll use Toast UI Calendar's default buttons and override their handlers
  } else if (event.calendarId === 'class_session') {
...
const setupPopupClickListeners = () => {
  ...
  calendarEl.addEventListener('click', async (e) => {
    ...
    // 🔥 REMOVED: Custom Edit/Delete button handlers
    // Now handled via calendarInstance.on('beforeUpdateEvent') and ('beforeDeleteEvent')
...
// 🔥 NEW: Handle Edit placement test
const handleEditPlacementTest = async (event) => {
  try {
    // Load full event data
    const response = await api.get(`/api/calendar/events/${event.id}`);
    if (!response.data.success) {
      await swal.error('Không thể tải thông tin lịch test');
      return;
    }
    ...
    if (!customerInfo) {
      await swal.error('Không tìm thấy thông tin khách hàng');
      return;
    }
    ...
    const result = await swal.fire({
      title: 'Chỉnh sửa lịch test đầu vào',
      html: `
        <div class="text-left space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Thời gian test *</label>
            <input type="datetime-local" id="test_date" class="swal2-input w-full" value="${localDateTime}" required>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Thời lượng (phút)</label>
            <input type="number" id="duration_minutes" class="swal2-input w-full" value="${Math.round((new Date(eventData.end_date) - new Date(eventData.start_date)) / 60000)}" min="30" max="240">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Địa điểm</label>
            <input type="text" id="location" class="swal2-input w-full" value="${eventData.location || ''}" placeholder="Ví dụ: Cơ sở Thống Nhất">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Ghi chú</label>
            <textarea id="notes" class="swal2-textarea w-full" placeholder="Ghi chú thêm...">${eventData.description || ''}</textarea>
          </div>
        </div>
      `,
      ...
      preConfirm: () => {
        ...
        if (!test_date) {
          swal.showValidationMessage('Vui lòng chọn thời gian test');
          return false;
        }
    ...
    if (result.isConfirmed) {
      try {
        ...
        const payload = {
          start_date: new Date(result.value.test_date).toISOString(),
          end_date: new Date(new Date(result.value.test_date).getTime() + (parseInt(result.value.duration_minutes) || 60) * 60000).toISOString(),
          location: result.value.location || null,
          description: result.value.notes || null,
        };

        const updateResponse = await api.put(`/api/calendar/events/${event.id}`, payload);
        if (updateResponse.data.success) {
          await swal.success('Đã cập nhật lịch test thành công!', 'Khách hàng đã nhận thông báo qua Zalo');
          ...
          // Reload calendar after a short delay to ensure popup is closed
          setTimeout(() => {
            loadEvents();
          }, 300);
        }
        ...
        await swal.error(error.response?.data?.message || 'Không thể cập nhật lịch test');
      }
    }
  } catch (error) {
    console.error('Failed to edit placement test:', error);
    await swal.error('Không thể tải thông tin lịch test');
  }
};

// 🔥 NEW: Handle Delete placement test
const handleDeletePlacementTest = async (event) => {
  console.log('[CalendarView] handleDeletePlacementTest called', event);
  
  try {
    const customerInfo = event.raw?.customer_info;
    const customerName = customerInfo?.name || 'khách hàng';
    
    console.log('[CalendarView] Customer info:', customerInfo, 'Name:', customerName);
    
    const result = await swal.fire({
      title: 'Xác nhận xóa lịch test',
      html: `Bạn có chắc chắn muốn xóa lịch test đầu vào cho <strong>${customerName}</strong>?<br/><br/>Khách hàng sẽ nhận thông báo qua Zalo về việc hủy lịch test.`,
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      try {
        const deleteResponse = await api.delete(`/api/calendar/placement-test/${event.id}/delete`);
        if (deleteResponse.data.success) {
          ...
          await swal.success('Đã xóa lịch test thành công!', 'Khách hàng đã nhận thông báo qua Zalo');
        ...
        await swal.error(error.response?.data?.message || 'Không thể xóa lịch test');
        throw error; // Re-throw to be caught by outer catch
      }
    }
  } catch (error) {
    console.error('Failed to delete placement test:', error);
    // Only show error if it's not a user cancellation
    if (error?.response) {
      await swal.error('Không thể xóa lịch test');
    }
    // Re-throw to prevent promise rejection issues
    throw error;
  }
};
...
const moveToToday = () => {
  // Navigate to daily calendar view
  router.push({ name: 'calendar.today' });
};
