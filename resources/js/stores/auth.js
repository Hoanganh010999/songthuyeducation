...
export const useAuthStore = defineStore('auth', {
    ...
    getters: {
        ...
        isStudent: (state) => {
            return state.user?.is_student || false;
        },

        isParent: (state) => {
            return state.user?.is_parent || false;
        },
        ...
        async login(credentials) {
            ...
            try {
                ...
                if (response.data.success) {
                    ...
                    // Save or remove current branch from localStorage
                    if (this.user.current_branch_id) {
                        localStorage.setItem('current_branch_id', this.user.current_branch_id);
                    } else {
                        // IMPORTANT: Remove old branch_id if user has no branch
