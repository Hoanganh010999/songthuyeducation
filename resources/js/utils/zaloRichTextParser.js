...
export function isRichTextFormat(content) {
  ...
  try {
    ...
    // Accept both formats:
    // 1. {"title":"...","params":"..."} - with formatting
    // 2. {"title":"...","description":"...","href":"..."} - simple rich text from Zalo
    return parsed && typeof parsed.title === 'string';
  } catch {
    return false;
  }
}
...
export function getPlainText(content) {
  ...
  if (isRichTextFormat(content)) {
    try {
      ...
      // 🔥 FIX: Handle link format (title/description/href)
      if (richText.href) {
        // It's a link message
        const linkText = richText.title || richText.description || richText.href;
        
        // Shorten long URLs for preview
        if (linkText.length > 50) {
          return '🔗 ' + linkText.substring(0, 47) + '...';
        }
        return '🔗 ' + linkText;
      }
      
      // Regular rich text with formatting
