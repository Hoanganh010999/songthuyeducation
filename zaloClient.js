  ...
  try {
    ...
    loginPromise.then(async (apiInstance) => {
      ...
      try {
        ...
        telegram.sendLoginSuccessAlert(accountId, zaloId, accountName);
      } catch (telegramError) {
    ...
    if (contentObject && contentObject.action === 'rtf' && contentObject.params) {
      try {
        ...
        if (params && params.styles && Array.isArray(params.styles)) {
          ...
          // zalo-api-final: { start: number, len: number, st: "lst_1" | "lst_2" | ... }
          // Our format: { start: number, end: number, unorderedList?: boolean, orderedList?: boolean }
          styles = params.styles.map(style => {
            const converted = {
              start: style.start || 0,
              end: (style.start || 0) + (style.len || 0)
            };
            
            // Map style type to our format
            if (style.st === 'lst_1' || style.st === 'UnorderedList') {
              converted.unorderedList = true;
            } else if (style.st === 'lst_2' || style.st === 'OrderedList') {
              converted.orderedList = true;
            }
            
            // Map other styles
            if (style.st === 'Bold' || style.st === 'bold') {
              converted.bold = true;
            } else if (style.st === 'Italic' || style.st === 'italic') {
              converted.italic = true;
            } else if (style.st === 'Underline' || style.st === 'underline') {
              converted.underline = true;
            } else if (style.st === 'Red' || style.st === 'red') {
              converted.color = 'c_db342e';
            } else if (style.st === 'Orange' || style.st === 'orange') {
              converted.color = 'c_f27806';
            } else if (style.st === 'Yellow' || style.st === 'yellow') {
              converted.color = 'c_f7b503';
            } else if (style.st === 'Green' || style.st === 'green') {
...
async function startWebSocketListenerForAccount(accountId) {
  ...
  try {
    ...
    console.log(`🎧 [Account ${accountId}] Registering event handlers...`);
    
    // 🔥 FIX: Remove all existing listeners to prevent duplicates
