...
const initCalendar = () => {
  ...
  calendarInstance = new Calendar(calendarContainer.value, {
    ...
    template: {
      ...
      // 🔥 NEW: Disable default Edit/Delete buttons for placement_test
      popupDetailButtons(event) {
        // Return empty string to hide default buttons for placement_test
        if (event.calendarId === 'placement_test') {
          return '';
        }
        // Return default buttons for other event types
        return '';
      },
      time(event) {
...
const getCustomPopupDetailBody = (event) => {
  ...
  if (event.calendarId === 'placement_test') {
    ...
    // 🔥 NEW: Add Edit and Delete buttons for placement test
    const canEditDelete = authStore.hasPermission('calendar.update') || authStore.hasPermission('calendar.delete');
    if (canEditDelete) {
      actionButtons += `
        <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb; display: flex; gap: 8px;">
          <a 
            href="#"
            class="edit-placement-test-btn"
            data-event-id="${event.id}"
            style="color: #3b82f6; text-decoration: none; font-size: 13px; font-weight: 500; cursor: pointer; display: flex; align-items: center; gap: 4px;"
          ...
          <a 
            href="#"
            class="delete-placement-test-btn"
            data-event-id="${event.id}"
            style="color: #ef4444; text-decoration: none; font-size: 13px; font-weight: 500; cursor: pointer; display: flex; align-items: center; gap: 4px;"
            onmouseover="this.style.textDecoration='underline'"
            onmouseout="this.style.textDecoration='none'">
            🗑️ Delete
          </a>
...
const setupPopupClickListeners = () => {
  ...
  calendarEl.addEventListener('click', async (e) => {
    ...
    // 🔥 NEW: Handle Edit placement test button
    const editPlacementTestBtn = e.target.closest('.edit-placement-test-btn');
    if (editPlacementTestBtn) {
      e.preventDefault();
      e.stopPropagation();
      
      const eventId = editPlacementTestBtn.getAttribute('data-event-id');
      if (eventId) {
        const event = allEvents.value.find(ev => ev.id == eventId);
        if (event && event.calendarId === 'placement_test') {
          handleEditPlacementTest(event);
        }
      }
      return;
    }
    
    // 🔥 NEW: Handle Delete placement test button
    const deletePlacementTestBtn = e.target.closest('.delete-placement-test-btn');
    if (deletePlacementTestBtn) {
      e.preventDefault();
      e.stopPropagation();
      
      console.log('[CalendarView] Delete button clicked', deletePlacementTestBtn);
      
      const eventId = deletePlacementTestBtn.getAttribute('data-event-id');
      console.log('[CalendarView] Event ID:', eventId);
      
      if (eventId) {
        const event = allEvents.value.find(ev => ev.id == eventId);
        console.log('[CalendarView] Found event:', event);
        
        if (event && event.calendarId === 'placement_test') {
          console.log('[CalendarView] Calling handleDeletePlacementTest');
          handleDeletePlacementTest(event);
        } else {
          console.warn('[CalendarView] Event not found or not placement_test', { event, calendarId: event?.calendarId });
        }
      } else {
        console.warn('[CalendarView] No event ID found');
      }
      return;
    }
