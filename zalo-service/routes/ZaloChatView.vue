      ...
      <div v-if="showEditor" class="mb-2 border border-gray-300 rounded-lg bg-white shadow-sm">
        ...
        <div class="flex items-center gap-1 p-2 border-b border-gray-200 bg-gray-50">
          ...
          <button
            @click="clearStyles"
            type="button"
            class="p-1.5 rounded hover:bg-gray-200 ml-auto"
            :title="t('zalo.clear_formatting') || 'Xóa định dạng'"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <div
          ...
          @keydown.enter.shift.exact="insertLineBreak"
...
const extractStylesFromEditor = () => {
  ...
  const traverse = (node, startIndex = 0) => {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      // Skip list items (li) - we handle them through their text nodes
      const tagName = node.tagName?.toLowerCase();
      if (tagName === 'ul' || tagName === 'ol' || tagName === 'li') {
        // Process children but don't add extra index for the list element itself
...
const sendMessage = async () => {
  ...
  if (showEditor.value && editorContent.value && text) {
    ...
    console.log('📤 [ZaloChatView] Extracted styles:', styles);
  }
