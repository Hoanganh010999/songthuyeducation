...
// ✅ Import pendingMessageMetadata từ shared module
const { pendingMessageMetadata } = require('../services/message-metadata');
...
router.post('/send', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] POST /api/message/send received');
    console.log('📥 [zalo-service] FULL REQUEST BODY:', JSON.stringify(req.body, null, 2));
    console.log('📥 [zalo-service] Request body summary:', {
      to: req.body.to,
      type: req.body.type,
    ...
    if (!to || !message) {
      console.error('❌ [zalo-service] Missing required fields:', {
        hasTo: !!to,
        hasMessage: !!message,
      });
    ...
    try {
      ...
      console.log('✅ [zalo-service] Zalo client retrieved');
      ...
      console.error('❌ [zalo-service] Failed to get Zalo client:', error.message);
    ...
    // Extract styles and mentions from request body (if sent from Laravel)
    const styles = req.body.styles;
    const mentions = req.body.mentions;
    
    // ✅ EXTRACT METADATA from request body (sent_by_user_id, etc.)
    const metadata = req.body.metadata;
    
    console.log('📤 [zalo-service] Sending message with params:', {
      type,
      to,
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
      messageLength: typeof message === 'string' ? message.length : (message?.msg?.length || 0),
      hasStyles: !!(styles && Array.isArray(styles) && styles.length > 0),
      hasMentions: !!(mentions && Array.isArray(mentions) && mentions.length > 0),
      hasMetadata: !!metadata,
      metadata: metadata,
    });
    
    // Format message according to zalo-api-final documentation:
    // - If message is already an object, use it as-is
    // - If message is a string AND we have styles/mentions, create MessageContent object
    // - If message is a string AND no styles/mentions, send as plain string
    let formattedMessage;
    
    if (typeof message === 'object' && message !== null) {
      // Message is already an object (MessageContent), use it directly
      formattedMessage = message;
      console.log('📤 [zalo-service] Using message object directly');
    } else if (typeof message === 'string') {
      // Check if we have styles or mentions
      ...
      if (hasStyles || hasMentions) {
        // Create MessageContent object with msg, styles, and mentions
        formattedMessage = {
          msg: message,
        };
        
        if (hasStyles) {
          formattedMessage.styles = styles;
          console.log('📤 [zalo-service] Adding styles to message:', {
            stylesCount: styles.length,
            stylesPreview: styles.slice(0, 2),
          });
        }
        
        if (hasMentions) {
          formattedMessage.mentions = mentions;
          console.log('📤 [zalo-service] Adding mentions to message:', {
            mentionsCount: mentions.length,
            mentionsPreview: mentions.slice(0, 2),
          });
        }
        
        console.log('📤 [zalo-service] Created MessageContent object with styles/mentions');
      } else {
        // Plain string message, no formatting needed
        formattedMessage = message;
        console.log('📤 [zalo-service] Using plain string message (no styles/mentions)');
      }
    } else {
      // Invalid message type, log error but try to send anyway
      console.warn('⚠️ [zalo-service] Unexpected message type:', typeof message);
      formattedMessage = message;
    }
    
    // Call sendMessage with correct signature: sendMessage(message, threadId, type)
    console.log('📤 [zalo-service] Calling zalo.sendMessage with:', {
      messageType: typeof formattedMessage,
      isMessageObject: typeof formattedMessage === 'object',
      hasMsg: typeof formattedMessage === 'object' ? !!formattedMessage.msg : false,
      hasStyles: typeof formattedMessage === 'object' ? !!formattedMessage.styles : false,
      hasMentions: typeof formattedMessage === 'object' ? !!formattedMessage.mentions : false,
      messageLength: typeof formattedMessage === 'string' ? formattedMessage.length : (formattedMessage?.msg?.length || 0),
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
    });
    
    const result = await zalo.sendMessage(formattedMessage, threadId, threadType);
    ...
    // ✅ SAVE METADATA to memory Map (if provided and msgId OR cliMsgId exists)
    // Use cliMsgId as primary key, fallback to msgId if cliMsgId is null
    const metadataKey = cliMsgId || msgId;
    
    if (metadata && metadataKey) {
      pendingMessageMetadata.set(metadataKey, {
        ...metadata,
        timestamp: Date.now(),
        cliMsgId: cliMsgId,
        msgId: msgId,
      ...
      console.log('💾 [zalo-service] Stored metadata in memory:', {
        key: metadataKey,
        key_type: cliMsgId ? 'cliMsgId' : 'msgId',
        cliMsgId: cliMsgId,
        msgId: msgId,
    ...
    } else if (metadata && !metadataKey) {
      console.warn('⚠️  [zalo-service] Metadata provided but no msgId/cliMsgId, cannot store:', {
        metadata: metadata,
        msgId: msgId,
        cliMsgId: cliMsgId,
      });
    ...
    console.log('✅ [zalo-service] Message sent successfully:', {
      ...
      metadata_stored: !!metadata && !!cliMsgId,
    });
    ...
    console.error('❌ [zalo-service] Send message error:', {
...
router.post('/send-image', verifyApiKey, async (req, res) => {
  ...
  try {
    ...
    console.log('📥 [zalo-service] POST /api/message/send-image received', {
      to,
    ...
    if (isAbsolutePath) {
      ...
      console.log('✅ [zalo-service] Using absolute path directly (no download):', finalImagePath);
    } else {
      // Download from URL to temporary file
      console.log('📥 [zalo-service] Downloading image from URL:', imageSource);
      ...
      try {
        ...
        console.warn('⚠️  Failed to extract extension from URL, using .jpg');
      }
      
      const fileName = `image_${Date.now()}_${Math.random().toString(36).substring(7)}${ext}`;
      tempFilePath = path.join(tempDir, fileName);
      
      console.log('📝 [zalo-service] Temp file will be saved as:', {
        fileName,
        extension: ext,
      ...
      console.log('✅ [zalo-service] Image downloaded to:', finalImagePath);
    }
    ...
    console.log('📤 [zalo-service] Sending image with:', {
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
    ...
    console.log('📤 [zalo-service] Sending message (zalo-api-final will auto-upload)...');
    ...
    console.log('📤 [zalo-service] Message content:', {
      hasMsg: true,
      hasAttachments: true,
    ...
    if (result?.attachment && Array.isArray(result.attachment) && result.attachment.length > 0) {
      ...
      console.log('📎 [zalo-service] Extracted from result.attachment:', {
        hasAttachment: true,
      ...
      console.warn('⚠️  [zalo-service] No attachment in result (will rely on WebSocket)');
    }
    ...
    console.log('✅ [zalo-service] Image sent successfully:', {
      hasResult: !!result,
    ...
    if (msgId && uploadedCdnUrl) {
      ...
      console.log('📝 [zalo-service] Stored image URL mapping:', {
        msgId,
    ...
    if (!isAbsolutePath && tempFilePath && fs.existsSync(tempFilePath)) {
      try {
        ...
        console.log('🧹 [zalo-service] Temporary image file deleted:', tempFilePath);
      } catch (cleanupError) {
        console.warn('⚠️  [zalo-service] Failed to delete temp file:', cleanupError.message);
      }
      tempFilePath = null;
    } else if (isAbsolutePath) {
      console.log('✅ [zalo-service] No cleanup needed - used absolute path directly');
    }
    ...
    console.error('❌ [zalo-service] Send image error:', {
      message: error.message,
      stack: error.stack,
      error: error,
    });
    ...
    if (!isAbsolutePath && tempFilePath && fs.existsSync(tempFilePath)) {
      try {
        await unlink(tempFilePath);
        console.log('🧹 [zalo-service] Temporary image file deleted after error:', tempFilePath);
      } catch (cleanupError) {
        console.warn('⚠️  [zalo-service] Failed to delete temp file after error:', cleanupError.message);
      }
    }
...
router.post('/send-file', verifyApiKey, async (req, res) => {
  ...
  try {
    ...
    console.log('📥 [zalo-service] POST /api/message/send-file received', {
      to,
      hasFileUrl: !!fileUrl,
    ...
    if (isAbsolutePath) {
      ...
      console.log('✅ [zalo-service] Using absolute file path:', finalFilePath);
    } else {
      // Download from URL to temporary file
      console.log('📥 [zalo-service] Downloading file from URL:', fileSource);
      ...
      console.log('✅ [zalo-service] File downloaded to:', finalFilePath);
    }
    ...
    console.log('📤 [zalo-service] Sending file with:', {
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
      fileName,
    ...
    console.log('📤 [zalo-service] Sending file message (attachment only)...');
    const result = await zalo.sendMessage(messageContent, threadId, threadType);
    ...
    console.log('✅ [zalo-service] File sent successfully:', {
      fileName,
      msgId,
      cliMsgId,
      globalMsgId,
    ...
    if (!isAbsolutePath && tempFilePath) {
      try {
        await unlink(tempFilePath);
        console.log('🧹 [zalo-service] Temporary file deleted:', tempFilePath);
      } catch (cleanupError) {
        console.warn('⚠️  [zalo-service] Failed to delete temp file:', cleanupError.message);
      }
    }
    ...
    console.error('❌ [zalo-service] Send file error:', {
      message: error.message,
      stack: error.stack,
    });
    ...
    if (!isAbsolutePath && tempFilePath && fs.existsSync(tempFilePath)) {
      try {
        await unlink(tempFilePath);
        console.log('🧹 [zalo-service] Temporary file deleted after error:', tempFilePath);
      } catch (cleanupError) {
        console.warn('⚠️  [zalo-service] Failed to delete temp file after error:', cleanupError.message);
      }
    }
...
router.post('/reply', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] POST /api/message/reply received');
    ...
    if (extractedQuote) {
      ...
      if (!cliMsgId && !globalMsgId) {
        console.warn('⚠️  [zalo-service] Quote data missing required fields (cliMsgId or globalMsgId), skipping quote');
      } else if (!msg) {
        console.warn('⚠️  [zalo-service] Quote data missing msg field, skipping quote');
      } else if (!ownerId) {
        console.warn('⚠️  [zalo-service] Quote data missing ownerId field, skipping quote');
      } else {
        ...
        console.log('📎 [zalo-service] Replying with quote (zalo-api-final format):', {
    ...
    console.log('📤 [zalo-service] Sending reply message:', {
      type,
      to,
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
    ...
    console.log('✅ [zalo-service] Reply message sent successfully');
    ...
    console.error('❌ [zalo-service] Reply message error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
router.post('/send-sticker', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] POST /api/message/send-sticker received');
    ...
    console.log('🎨 [zalo-service] Sending sticker with:', {
      threadId,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
    ...
    console.log('✅ [zalo-service] Sticker sent successfully:', {
      msgId,
      cliMsgId,
      hasResult: !!result,
      resultKeys: result ? Object.keys(result) : [],
    ...
    console.error('❌ [zalo-service] Send sticker error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
router.get('/stickers', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] GET /api/message/stickers received');

    const { keyword = 'happy' } = req.query; // Default to 'happy' for popular stickers

    // 🔧 FIX: Convert accountId to integer (headers are always strings)
    let accountId = req.headers['x-account-id'];
    if (accountId) {
      accountId = parseInt(accountId);
    }
    ...
    console.log('🔍 [zalo-service] Searching stickers with keyword:', keyword);

    // Step 1: Search for sticker IDs by keyword
    const stickerIds = await zalo.getStickers(keyword);

    console.log('📋 [zalo-service] Found sticker IDs:', stickerIds?.length || 0);
    ...
    console.log('✅ [zalo-service] Retrieved sticker details:', stickers?.length || 0);
    ...
    console.error('❌ [zalo-service] Get stickers error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
router.post('/reaction', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] POST /api/message/reaction received');
    ...
    if (typeof reaction === 'string') {
      ...
      if (reactionIconMap[reaction]) {
        ...
        console.log(`📝 [zalo-service] Mapped reaction "${reaction}" to Reactions enum:`, {
          original: reaction,
          mapped: reactionIcon,
      ...
      // Then try Reactions enum directly (e.g., 'HEART' -> Reactions.HEART)
        ...
        console.log(`📝 [zalo-service] Found reaction "${reaction}" in Reactions enum:`, {
          original: reaction,
          found: reactionIcon,
      ...
      // Otherwise, use the string as-is (Zalo might accept it directly)
        ...
        console.log(`⚠️  [zalo-service] Reaction "${reaction}" not found in Reactions enum, using as-is`);
      }
    }
    ...
    if (!validReactions.includes(reactionIcon) && typeof reactionIcon === 'string') {
      console.warn(`⚠️  [zalo-service] Reaction icon "${reactionIcon}" may not be a valid Reactions enum value`);
    }
    ...
    console.log('😊 [zalo-service] Adding reaction:', {
    ...
    if (destination.data.cliMsgId === destination.data.msgId) {
      console.warn('⚠️  [zalo-service] cliMsgId and msgId are the same:', {
    ...
    console.log('📤 [zalo-service] Calling zalo.addReaction with:', {
      icon: reactionIcon,
    ...
    try {
      ...
      console.log('✅ [zalo-service] Reaction added successfully:', {
        hasResult: !!result,
      ...
      console.error('❌ [zalo-service] addReaction call failed:', {
    ...
    console.error('❌ [zalo-service] Add reaction error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
/**
 * POST /api/message/create-reminder
 * Create a reminder/event in a conversation
 * Body: { thread_id, title, start_time, type, emoji?, repeat? }
 */
router.post('/create-reminder', verifyApiKey, async (req, res) => {
  try {
    ...
    if (!thread_id || !title || !start_time) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: thread_id, title, start_time'
      });
    }
    ...
    let startTimeMs = start_time;
    if (typeof start_time === 'string') {
      startTimeMs = new Date(start_time).getTime();
    }
    ...
    if (isNaN(startTimeMs) || startTimeMs < Date.now()) {
      return res.status(400).json({
        success: false,
        message: 'Invalid start_time. Must be a valid future timestamp or ISO date string.'
      });
    }

    const threadType = type === 'group' ? ThreadType.Group : ThreadType.User;

    console.log('📅 [zalo-service] Creating reminder:', {
      thread_id,
      title,
    ...
    const reminderOptions = {
      ...
      emoji: emoji,
      repeat: repeat, // 0 = None, 1 = Daily, 2 = Weekly, 3 = Monthly, etc.
    };

    const result = await zalo.createReminder(reminderOptions, thread_id, threadType);

    console.log('✅ [zalo-service] Reminder created successfully:', {
      hasResult: !!result,
      resultKeys: result ? Object.keys(result) : [],
    });
    ...
    console.error('❌ [zalo-service] Create reminder error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
/**
 * GET /api/message/reminders
 * Get list of reminders for a conversation
 * Query: { thread_id, type }
 */
router.get('/reminders', verifyApiKey, async (req, res) => {
  try {
    console.log('📥 [zalo-service] GET /api/message/reminders received');

    const { thread_id, type = 'user' } = req.query;
    ...
    console.log('📅 [zalo-service] Getting reminders for:', {
      thread_id,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
    });

    // Call getListReminder API
    const result = await zalo.getListReminder(thread_id, threadType);

    console.log('✅ [zalo-service] Reminders retrieved:', {
      hasResult: !!result,
    ...
    console.error('❌ [zalo-service] Get reminders error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
...
router.post('/send-video', verifyApiKey, async (req, res) => {
  ...
  try {
    ...
    console.error('❌ [zalo-service] Send video error:', {
      message: error.message,
      stack: error.stack,
    });
 ...
 * Body: { to, voiceUrl, type, ttl? }
 */
router.post('/send-voice', verifyApiKey, async (req, res) => {
  try {
    ...
    if (!to || !voiceUrl) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: to, voiceUrl'
      });
    }
    ...
    console.log('🎤 [zalo-service] Sending voice:', {
      to,
      voiceUrl,
      threadType: threadType === ThreadType.Group ? 'Group' : 'User',
      ttl,
    ...
    const result = await zalo.sendVoice(voiceOptions, to, threadType);

    console.log('✅ [zalo-service] Voice sent successfully:', {
      hasResult: !!result,
      msgId: result?.msgId,
    });
    ...
    console.error('❌ [zalo-service] Send voice error:', {
      message: error.message,
      stack: error.stack,
    });
    res.status(500).json({
