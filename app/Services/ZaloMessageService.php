...
class ZaloMessageService
{
    ...
    public function saveSentMessage(
        ...
        try {
            ...
            // 🎨 Extract styles from rich text message JSON format (for sent messages)
                ...
                if ($contentJson && isset($contentJson['action']) && $contentJson['action'] === 'rtf' && isset($contentJson['params'])) {
                    try {
                        ...
                        if ($params && isset($params['styles']) && is_array($params['styles'])) {
                            // ✅ PRESERVE original style format with 'st' field
                            // This ensures styles display correctly in frontend
                            $styles = [];
                            foreach ($params['styles'] as $style) {
                                $st = $style['st'] ?? '';

                                // Normalize enum names to style codes if needed
                                if ($st === 'UnorderedList') {
                                    $st = 'lst_1';
                                } elseif ($st === 'OrderedList') {
                                    $st = 'lst_2';
                                } elseif ($st === 'Bold') {
                                    $st = 'b';
                                } elseif ($st === 'Italic') {
                                ...
                                // ✅ KEEP st field - this is what frontend needs to display styles
                                $styles[] = [
                                    'start' => $style['start'] ?? 0,
                                    'len' => $style['len'] ?? 0,
                                    'st' => $st  // ← CRITICAL: Preserve st field!
                                ];
                            }
                            ...
                            Log::info('[ZaloMessageService] Extracted styles from sent rich text message', [
                                ...
                                'styles' => $styles,
                                'has_list_styles' => !empty(array_filter($styles, fn($s) => in_array($s['st'] ?? '', ['lst_1', 'lst_2']))),
                                'has_color_styles' => !empty(array_filter($styles, fn($s) => str_starts_with($s['st'] ?? '', 'c_'))),
                            ]);
                        }
                    } catch (\Exception $e) {
            ...
            // 🎨 Detect if content is sticker JSON format
            ...
            // 🎨 Add styles to metadata if provided
            ...
            // 🎨 Add sticker data to metadata if provided
            if ($stickerData) {
            ...
            // 🔥 DEBUG: Log cliMsgId before cache lookup
            Log::info('[ZaloMessageService] About to lookup cache', [
                'has_cliMsgId' => !empty($cliMsgId),
                'cliMsgId' => $cliMsgId,
                'cache_key' => $cliMsgId ? "zalo_msg_{$cliMsgId}_user" : null,
            ]);
            
            // 🔥 LOOKUP user_id from cache (set by sendMessage())
            // This enables WebSocket to save sent_by_user_id while keeping sync with Zalo
            $sentByUserId = null;
            if ($cliMsgId) {
                $cacheKey = "zalo_msg_{$cliMsgId}_user";
                $userInfo = \Cache::get($cacheKey);
                
                if ($userInfo && isset($userInfo['user_id'])) {
                    $sentByUserId = $userInfo['user_id'];
                    Log::info('[ZaloMessageService] Retrieved user_id from cache', [
                        'cache_key' => $cacheKey,
                        'user_id' => $sentByUserId,
                        'user_name' => $userInfo['user_name'] ?? null,
                    ]);
                    
                    // Delete cache after use to prevent memory leak
                    \Cache::forget($cacheKey);
                } else {
                    Log::warning('[ZaloMessageService] No user_id in cache for sent message', [
                        'cache_key' => $cacheKey,
                        'cliMsgId' => $cliMsgId,
                        'cache_value' => $userInfo,
                    ]);
                }
            }
            ...
            $message = ZaloMessage::updateOrCreate(
                ...
                [
                    ...
                    'recipient_id' => $recipientId,
                    'recipient_name' => $recipientName,
                    'sent_by_user_id' => $sentByUserId, // 🔥 KEY: Save user who sent (from cache)
                    'content' => $content,
                    'content_type' => $contentType,
                    'media_url' => $mediaUrl,
            ...
            // 🎯 Auto-record sticker usage for recent stickers list
    ...
    public function saveReceivedMessage(
        ...
        ?string $recipientId = null, // 🔥 FIX: Add recipient ID parameter (group ID for group messages)
        bool $isSelf = false // 🔥 NEW: Flag to indicate self-sent message
    ): ZaloMessage {
        ...
        // 🎨 Extract styles from rich text message JSON format
        // Format: {"title":"text","action":"rtf","params":"{\"styles\":[...]}"}
        if (!$styles && $contentType === 'text' && !empty($content)) {
            $contentJson = json_decode($content, true);
            if ($contentJson && isset($contentJson['action']) && $contentJson['action'] === 'rtf' && isset($contentJson['params'])) {
        ...
        // 🎨 Detect if content is sticker JSON format (same as saveSentMessage)
        if ($contentType === 'text' && !empty($content)) {
            $decoded = json_decode($content, true);
            if (is_array($decoded) && isset($decoded['id']) && isset($decoded['catId']) && isset($decoded['type'])) {
                // This is a sticker sent as text - convert it
        ...
        // 📁 Add file data to metadata if provided
        if ($fileData) {
        ...
        // 🔥 FIX: For group messages, use recipientId (group ID) not senderId
        ...
        // 🔧 FIX: Use Laravel's now() timestamp for accuracy
        ...
        // 🔥 PRAGMATIC FIX: If this is a self-sent message (isSelf=true), lookup sent_by_user_id from cache
        // This handles the case where saveReceivedMessage is called instead of saveSentMessage
        $sentByUserId = null;
        if ($isSelf && $cliMsgId) {
            $cacheKey = "zalo_msg_{$cliMsgId}_user";
            $userInfo = \Cache::get($cacheKey);
            ...
            if ($userInfo && isset($userInfo['user_id'])) {
                $sentByUserId = $userInfo['user_id'];
                Log::info('[ZaloMessageService] Retrieved user_id from cache (in saveReceivedMessage)', [
                    'cache_key' => $cacheKey,
                    'user_id' => $sentByUserId,
                    'user_name' => $userInfo['user_name'] ?? null,
                    'is_self' => $isSelf,
                ...
                Log::warning('[ZaloMessageService] No user_id in cache for self-sent message (in saveReceivedMessage)', [
                    'cache_key' => $cacheKey,
                    'cliMsgId' => $cliMsgId,
                    'is_self' => $isSelf,
                ]);
        ...
        $savedMessage = ZaloMessage::updateOrCreate(
            ...
            [
                'type' => $isSelf ? 'sent' : 'received', // 🔥 FIX: Use 'sent' if isSelf=true
                'recipient_type' => $recipientType,
                'recipient_id' => $actualRecipientId, // 🔥 FIX: Use group ID for groups, sender ID for users
                'recipient_name' => $actualRecipientName, // 🔥 FIX: Null for groups (will be fetched)
                'sender_id' => $actualSenderId ?? $senderId, // Actual sender ID
                'sender_name' => $actualSenderName ?? $senderName, // Actual sender name
                'sent_by_user_id' => $sentByUserId, // 🔥 KEY: Set user who sent (from cache if isSelf)
                'content' => $content,
                'content_type' => $contentType,
                'media_url' => $mediaUrl,
                'media_path' => $mediaPath,
                ...
                'quote_data' => $quote,
                'metadata' => $metadata,
                'created_at' => $zaloTimestamp, // 🔥 CRITICAL: Set created_at to Zalo timestamp
