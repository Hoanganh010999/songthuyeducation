...
class CustomerZaloNotificationService
{
    ...
    /**
     * 🔥 NEW: Format placement test updated message
     */
    protected function formatPlacementTestUpdatedMessage(
        CalendarEvent $event,
        Customer $customer,
        ?CustomerChild $child = null,
        $oldStartDate = null,
        ...
        $message = "📝 THÔNG BÁO THAY ĐỔI LỊCH TEST ĐẦU VÀO\n\n";
        ...
        $message .= "Lịch test đầu vào của bạn đã được cập nhật:\n\n";
        $message .= "📅 Thời gian mới: {$testDate->format('d/m/Y')}\n";
        $message .= "🕐 Giờ mới: {$testDate->format('H:i')}\n";
        ...
        if ($oldStartDate) {
            $oldDate = \Carbon\Carbon::parse($oldStartDate)->setTimezone($timezone);
            $message .= "\n📋 Thông tin cũ:\n";
            $message .= "   Thời gian: {$oldDate->format('d/m/Y H:i')}\n";
            if ($oldLocation) {
                $message .= "   Địa điểm: {$oldLocation}\n";
            }
        }
    ...
    /**
     * 🔥 NEW: Format placement test cancelled message
     */
    protected function formatPlacementTestCancelledMessage(
        array $eventData,
        Customer $customer,
        ?CustomerChild $child = null
    ): string {
        $timezone = $this->getConfiguredTimezone();
        $testDate = \Carbon\Carbon::parse($eventData['start_date'])->setTimezone($timezone);
        
        $message = "❌ THÔNG BÁO HỦY LỊCH TEST ĐẦU VÀO\n\n";
        ...
        $message .= "Chúng tôi xin thông báo rằng lịch test đầu vào của bạn đã bị hủy:\n\n";
        $message .= "📅 Thời gian đã đăng ký: {$testDate->format('d/m/Y')}\n";
        $message .= "🕐 Giờ đã đăng ký: {$testDate->format('H:i')}\n";

        if (isset($eventData['location']) && $eventData['location']) {
            $message .= "📍 Địa điểm: {$eventData['location']}\n";
        }

        $message .= "\nNếu bạn muốn đăng ký lại lịch test mới, vui lòng liên hệ lại cho chúng tôi.\n";
        $message .= "Chúng tôi xin lỗi vì sự bất tiện này! 🙏";
