...
const fetchTeachers = async () => {
  ...
  try {
    ...
    // Step 1: Load teacher position codes from settings
    ...
    if (positionCodes.length === 0) {
      console.warn('⚠️ No teacher position codes configured for this branch');
      teachers.value = [];
      return;
    }

    // Step 2: Fetch teachers based on position codes
    ...
    console.log('✅ Loaded teachers:', teachers.value.length, 'with position codes:', positionCodes);
  } catch (error) {
