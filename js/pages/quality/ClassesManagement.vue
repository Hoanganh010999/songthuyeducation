...
const loadClasses = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/classes', { params: { branch_id: branchId } });
...
const openZaloGroupChat = async (classItem) => {
  ...
  // Get default Zalo account ID (first active account)
  if (!classItem.zalo_account_id) {
    try {
      const response = await axios.get('/api/zalo/accounts');
      if (response.data.success && response.data.data.length > 0) {
        currentZaloAccountId.value = response.data.data[0].id;
      }
    } catch (error) {
