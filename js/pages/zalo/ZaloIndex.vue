        ...
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <button
              ...
              @click="activeNav === 'history' ? handleSyncHistory() : handleResync()"
              ...
              :title="activeNav === 'history' ? (t('zalo.sync_history') || 'Sync Chat History') : (t('zalo.resync_from_zalo') || 'Resync from Zalo')"
            >
              ...
              <span class="hidden md:inline">{{ activeNav === 'history' ? (t('zalo.sync_history') || 'Sync History') : (t('zalo.resync') || 'Resync') }}</span>
            </button>
          </div>
        </div>
        <div class="relative">
...
const loadList = async (forceSync = false) => {
  ...
  try {
    ...
    if (activeNav.value === 'history') {
      ...
      listItems.value = conversations.map(conv => ({
        ...conv,
      ...
      console.log('📥 [ZaloIndex] Loading friends for account:', accountId);
      params.sync = forceSync;
      const response = await axios.get('/api/zalo/friends', { params });
      listItems.value = (response.data.data || []).map(friend => ({
        ...friend,
    ...
    } else if (activeNav.value === 'groups') {
      if (accountId) {
        params.account_id = accountId;
      }
      console.log('📥 [ZaloIndex] Loading groups for account:', accountId);
      params.sync = forceSync;
      const response = await axios.get('/api/zalo/groups', { params });
      listItems.value = (response.data.data || []).map(group => ({
        ...group,
  ...
  const unsubscribeConversation = zaloSocket.onConversationUpdate((data) => {
    if (data.account_id === currentAccount.value?.id) {
      ...
      if (conversation) {
        ...
        if (data.last_message) conversation.last_message = data.last_message;
        ...
        // New conversation not in list, need to reload
        console.log('⚠️  [ZaloIndex] New conversation, reloading list');
        loadList();
      }

      // 🔥 NEW: Refresh total unread count when conversation updates
      fetchUnreadCounts();
    }
  });
  
  // 🔔 Listen for conversations updated event (after sync history fixes Unknown)
  const unsubscribeConversationsUpdated = zaloSocket.onConversationsUpdated((data) => {
    if (data.account_id === currentAccount.value?.id) {
