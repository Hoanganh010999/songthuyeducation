...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('account_items', function (Blueprint $table) {
            $table->id();
            $table->string('code', 50)->unique()->comment('Mã khoản');
            $table->string('name')->comment('Tên khoản thu chi');
            $table->foreignId('category_id')->constrained('account_categories')->onDelete('cascade')->comment('Hạng mục');
